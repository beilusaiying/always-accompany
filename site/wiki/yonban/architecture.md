...
## 工具执行状态机：不再只有"成功/失败/没反应"

每次工具调用是一个带 jobId 的 **Job**，全程有可观测状态：

```
tool_started → queued → sent → running → succeeded / failed
                     （中间态：wait_timeout / long_running / detached）
```

- **running / long_running**：命令跑了很久没结束不等于死了——长任务通知会显示真实的已运行时长，通知中心聚合展示所有未完成的工具 Job；
- **wait_timeout**：等待超时不再静默丢弃——**迟到的结果会回归它所属的对话**，不会因为等待方先超时就变成孤儿；
- 每个 Job 记录后端类型与端口（CLI 还是哪个编辑器窗口执行的），排查"是谁跑的"有据可查。
...
| 长时间没反应 | 先看通知中心的工具 Job 状态：running/long_running = 还在跑；wait_timeout = 等待超时（结果稍后仍会回归对话）；都没有 → 步骤 7 — WS 连接断开，检查连接面板状态灯 |
