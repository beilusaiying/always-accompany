...
/**
 * {{random::a,b,c}} 替换：每次求值都重新抽取（entropy:true 不固定种子），故同一文本多次渲染结果可不同。
 * 与 {{pick}} 的区别在于 pick 用内容+位置做种子，结果是稳定的。
 * @param {any} input 输入
...
/**
 * {{pick::a,b,c}} 替换：用「原文哈希 + 该宏在文本中的偏移」做种子，
 * 因此同一处 pick 在同一原文下每次都抽到同一项（确定性），不同位置各自独立。
 * @param {any} input 输入
 * @param {any} rawContent 原始内容（求哈希用，保证同一卡稳定）
 * @param {any} emptyListPlaceholder 空列表占位符
 * @returns {any} 替换后的字符串。
 */
