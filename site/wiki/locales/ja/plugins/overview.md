...
[プラグイン管理](beilu:settings/plugins)パネルで、現在読み込まれているプラグインを確認・設定できます。2026-08-03 時点のソースには 23 個の内蔵プラグインディレクトリがあり、新規ユーザーテンプレートは 14 個を列挙します。「ソースに存在」「読み込み済み」「内部機能が有効」は別の状態です。

> この日本語ページの個別一覧はまだ全 23 件へ展開されていません。最新の選択基準、既定状態、権限、データ送信先については [Plugin Manual (English)](../../en/plugins/overview.md) または[中国語版](../../../plugins/overview.md)を参照してください。
