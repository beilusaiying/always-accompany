...
import info from '../../../../public/parts/plugins/beilu-ejs/info.json' with { type: 'json' }; // T3a·3.3: part 元数据留原位（part 身份归 parts 树）
...
async function renderEjsInPromptStruct(prompt_struct, renderInputs) {
	...
	if (jobs.length === 0) return { rendered, skipped };
	...
	interfaces: {
		...
		chat: {
			...
			TweakPrompt: async (arg, prompt_struct, my_prompt, detail_level) => {
				...
				// 执行 EJS 渲染（沙箱）
				wbT(_wbChatId, 'ejs:tweakprompt', 'render_start', { hasStatData: !!statData });
				const stats = await renderEjsInPromptStruct(prompt_struct, renderInputs);
				wbT(_wbChatId, 'ejs:tweakprompt', 'render_done', { rendered: stats.rendered, skipped: stats.skipped });
				if (stats.rendered === 0) wbT(_wbChatId, 'ejs:tweakprompt', 'no_change', { skipped: stats.skipped });
