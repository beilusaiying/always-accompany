...
async function checkPythonDeps() {
  eyeStatus = "checking";
  setEyeProcessState({ status: eyeStatus });
  try {
    ...
    const command = new Deno.Command(pythonCmd, {
      args: [
        "-c",
        'import mss, pystray, keyboard; from PIL import Image; print("OK")',
      ],
      stdout: "piped",
      stderr: "piped",
    });
    ...
    eyeStatus = "error";
    setEyeProcessState({ status: eyeStatus, error: eyeError });
    return false;
  } catch (err) {
...
async function launchPythonEye() {
  ...
  try {
    ...
    console.log(
      "[beilu-eye] Python 桌面截图工具已启动 (PID:",
      eyeProcess.pid,
      ")",
    );
    ...
    config: {
      ...
      SetData: async (data) => {
        ...
        if (data._action === "clear") {
          const hadPending = hasPendingInjection();
          consumePendingInjection();
          console.log(
