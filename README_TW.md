...
- **分層的記憶與上下文** — Data + hot / warm / cold 分層儲存長期材料，一個收集上下文、檢索記憶的工具（P1）在每輪回答前召回目前相關的片段；上下文清理做到檔案讀取級顆粒度、可逆，AI 也能自行放棄不再需要的已讀檔案；
- **核心行為可查看、可設定** — 角色、提示詞、注入、記憶、召回路線、權限與外掛都有對應的編輯或設定入口；
- **以外掛為基礎的擴充框架** — 核心功能以外掛組織，經中間資訊站傳導，前端負責展示與操作；使用者外掛可用 JS、Python 或獨立程式撰寫；
- **一套整合的 agent 工具鏈** — 在同一套記憶與上下文框架下提供檔案、命令、瀏覽器整合、MCP、多視窗、審批與復原；實際可用範圍和效果取決於所選模式、設定、執行環境、模型與外部服務。
...
填入服務位址、API Key 和模型，儲存後選擇或匯入一張角色卡，就能開始聊天。至少需要一個可用的 AI API；模型能力與費用取決於你綁定的服務。應用程式內建 [Wiki](site/wiki/getting-started/overview.md)，也可存取[線上版](https://beilusaiying.github.io/always-accompany/)。
...
- **👑 主要提示詞條目可編輯**：角色定義、預設、INJ 條目、模式說明、記憶資料槽與工具說明，會依對應編輯器提供內容、順序、開關、身份、注入位置或條件設定；框架安全包裝和供應商特定的訊息轉換仍由程式碼管理；
...
把它們與 P1（一個收集上下文、檢索記憶的工具）組合起來，就形成「向量 + 動態注入 + 記憶跟著目前任務走」的可設定流程，檔案讀取級上下文清理也在同一條鏈路中。實際召回與壓縮效果仍取決於所選模式、設定、材料和模型。
...
<details>
<summary><strong>👑 主要提示詞條目可編輯 — 預設可用，也能按自己的流程設定</strong></summary>

角色定義、預設、INJ 條目、模式說明、記憶資料槽、工具說明等主要提示詞條目，可以在介面裡編輯。每類條目可調整其編輯器實際提供的選項；框架安全包裝和供應商特定的訊息轉換仍由程式碼管理：
...
**目前儲存庫已經具備的入口與實作**：Data + 三層記憶 · 上下文管理 · 自驅動 P1 / AI P1 · 提示詞條目編輯與預設切換 · 模式記憶表 · 條件知識動態注入 · Live2D / 圖片桌寵 · 螢幕感知與遊戲陪伴 · 本機語音輸入 · PPT 生成 · MCP · 多視窗 · VS Code 擴充橋 · 9 個平台 Bot · 23 個內建外掛目錄 · 使用者外掛宿主 · 回收 / 備份鏈 · 白盒診斷 · 多語言與主題。
...
always-accompany 不是為了把熱門功能塞進同一個選單——一開始只是作者自己想用 :)。它包含外掛與框架體系，並提供多種介面語言；實際涵蓋範圍會隨外掛和翻譯資源而變化。
