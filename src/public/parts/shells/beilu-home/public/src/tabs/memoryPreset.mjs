  ...
  try {
    ...
    try {
      const configResp = await fetch(
        `${SSM_API_BASE}/AI/${encodeURIComponent(sourceName)}`,
      );
      if (configResp.ok) {
        const sourceConfig = await configResp.json();
        sourceUrl =
          sourceConfig.config?.url || sourceConfig.config?.base_url || "";
        sourceKey = sourceConfig.config?.apikey || "";
      }
    } catch (e) {
      console.warn(
        "[memoryPreset] 获取源配置失败，回退到 sourceName 方式:",
        e.message,
      );
    }
    ...
    if (sourceUrl) {
      payload = {
        _action: "getModels",
        apiConfig: { url: sourceUrl, key: sourceKey },
      };
    } else {
    ...
    if (res.success && Array.isArray(res.models)) {
      ...
      if (select) {
        select.innerHTML =
          '<option value="" disabled selected>▼ 点击选择模型 (或在上方直接输入)</option>';
        res.models.forEach((model) => {
          const option = document.createElement("option");
          option.value = model;
    ...
    if (dom.apiModel) {
      ...
      setTimeout(() => {
        if (dom.apiModel && !dom.apiModel.value)
          dom.apiModel.placeholder = originalPlaceholder || "输入或选择模型";
      }, 2000);
    }
  }
}
...
async function setPluginData(payload) {
  try {
    ...
    const resp = await fetch(
      `/api/parts/plugins:${PLUGIN_NAME}/config/setdata`,
      {
        method: "POST",
...
function cacheDom() {
  ...
  dom.presetPreviewContent = document.getElementById(
    "mp-preset-preview-content",
  );
  ...
  dom.cfgChatHistoryCount = document.getElementById(
    "mp-cfg-chat-history-count",
  );
...
function renderPresetList() {
  ...
  for (const preset of presets) {
    ...
    item.className =
      "beilu-preset-entry-item" +
      (preset.id === selectedPresetId ? " active" : "");
...
function renderPromptList(preset) {
  ...
  if (!preset.prompts || preset.prompts.length === 0) {
    dom.promptList.innerHTML =
      '<p class="text-xs text-base-content/40 text-center py-4">暂无提示词条目</p>';
    return;
  }
  ...
  preset.prompts.forEach((prompt, idx) => {
    ...
    const isBuiltinMacro =
      prompt.builtin &&
      ["{{chat_history}}", "{{presetList}}"].includes(prompt.content);

    // 角色emoji
    const roleEmoji =
      prompt.role === "system" ? "🔧" : prompt.role === "user" ? "👤" : "🤖";
    ...
    if (isBuiltinMacro) {
      const builtinDescs = {
        "{{chat_history}}":
          "聊天记录注入占位符 — 此位置将插入实际对话记录，不可编辑",
        "{{presetList}}":
          "预设列表宏占位符 — 运行时将替换为可用预设的描述列表，不可编辑",
      };
  ...
  dom.promptList.querySelectorAll(".mp-prompt-entry").forEach((el) => {
    ...
    el.addEventListener("drop", async (e) => {
      ...
      // 重排 preset.prompts
      const dragIdx = preset.prompts.findIndex(
        (p) =>
          (p.identifier || `prompt_${preset.prompts.indexOf(p)}`) ===
          draggedPromptId,
      );
      if (dragIdx === -1) return;
      ...
      let targetIdx = preset.prompts.findIndex(
        (p) =>
          (p.identifier || `prompt_${preset.prompts.indexOf(p)}`) ===
          targetIdentifier,
      );
...
function renderInjectionList() {
  ...
  for (const inj of injectionPrompts) {
    ...
    headerDiv.className =
      "flex items-center justify-between cursor-pointer mp-injection-header";
    headerDiv.innerHTML = `
			<div class="flex items-center gap-2">
  ...
  // 展开/收起
  dom.injectionList
    .querySelectorAll(".mp-injection-header")
    .forEach((header) => {
  ...
  // 启用开关
  dom.injectionList
    .querySelectorAll(".mp-injection-toggle")
    .forEach((toggle) => {
  ...
  dom.injectionList.querySelectorAll(".mp-injection-preview").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const injId = e.target.dataset.injId;
      const previewArea = dom.injectionList.querySelector(
        `.mp-injection-preview-area[data-inj-id="${injId}"]`,
      );
      ...
      // 设置加载状态
      const contentEl = previewArea.querySelector(
        `.mp-preview-content[data-inj-id="${injId}"]`,
      );
      const statsEl = previewArea.querySelector(
        `.mp-preview-stats[data-inj-id="${injId}"]`,
      );
      ...
      try {
        ...
        if (statsEl)
          statsEl.textContent = `${(result.charCount || 0).toLocaleString()} 字符 · ≈${(result.estimatedTokens || 0).toLocaleString()} tokens`;

        // 热记忆部分
        const hotSection = previewArea.querySelector(
          `.mp-preview-hot-section[data-inj-id="${injId}"]`,
        );
        const hotContent = previewArea.querySelector(
          `.mp-preview-hot-content[data-inj-id="${injId}"]`,
        );
        const hotStats = previewArea.querySelector(
          `.mp-preview-hot-stats[data-inj-id="${injId}"]`,
        );
        ...
        if (result.hotMemoryPreview && result.hotMemoryCharCount > 0) {
          ...
          if (hotStats)
            hotStats.textContent = `${result.hotMemoryCharCount.toLocaleString()} 字符`;
        } else {
  ...
  dom.injectionList.querySelectorAll(".mp-preview-copy").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      e.stopPropagation();
      const injId = e.target.dataset.injId;
      const contentEl = dom.injectionList.querySelector(
        `.mp-preview-content[data-inj-id="${injId}"]`,
      );
      if (!contentEl) return;
      try {
...
function populateSourceSelect(currentValue) {
  ...
  if (aiSourceList.length === 0) {
    ...
    hint.style.cssText =
      "font-size:0.7rem;color:#d97706;margin-top:4px;padding:4px 6px;background:rgba(217,119,6,0.08);border-radius:4px;border:1px dashed rgba(217,119,6,0.3);";
    hint.innerHTML =
      '⚠️ 请先在 <a href="/parts/shells:beilu-home/#system" style="color:#d97706;text-decoration:underline;font-weight:500;">系统设置 → AI 服务源</a> 中添加服务源，记忆AI才能正常工作';
...
async function loadCharList() {
  ...
  try {
    ...
    dom.charSelect.innerHTML =
      '<option value="">选择角色卡查看宏数据...</option>';
    for (const key of charKeys) {
      const opt = document.createElement("option");
      opt.value = key;
      const details = cachedDetails[key];
      const displayName =
        details?.info?.display_name || details?.DisplayName || key;
      opt.textContent = displayName;
      dom.charSelect.appendChild(opt);
    }

    if (dom.charStatus)
      dom.charStatus.textContent = `${charKeys.length} 个角色卡`;
  } catch (err) {
...
function bindEvents() {
  ...
  dom.charSelect?.addEventListener("change", async () => {
    ...
    currentCharDisplayName =
      dom.charSelect.selectedOptions[0]?.textContent || "";
    if (dom.charStatus)
      dom.charStatus.textContent = currentCharId
        ? `已选: ${currentCharDisplayName || currentCharId}`
        : "";
    await refreshPresets();
  });

  // API 自定义开关
  dom.apiCustom?.addEventListener("change", async () => {
    ...
    // use_custom 切换即时保存，避免刷新后状态丢失
    if (selectedPresetId) {
      await setPluginData({
        _action: "updateMemoryPreset",
        presetId: selectedPresetId,
        api_config: { use_custom: dom.apiCustom.checked },
      });
    }
  });
  ...
  dom.saveBtn?.addEventListener("click", async () => {
    ...
    for (let i = 0; i < preset.prompts.length; i++) {
      ...
      if (
        p.builtin &&
        ["{{chat_history}}", "{{presetList}}"].includes(p.content)
      )
        continue;
  ...
  dom.archiveHotBtn?.addEventListener("click", async () => {
    ...
    try {
      ...
      showMaintenanceStatus(
        `✅ #7归档: ${result.remember_archived || 0} · #8归档: ${result.forever_archived || 0}`,
        4000,
      );
    } catch (e) {
      showMaintenanceStatus("❌ " + e.message, 4000);
  ...
  dom.previewPresetBtn?.addEventListener("click", async () => {
    ...
    if (dom.presetPreviewContent)
      dom.presetPreviewContent.innerHTML =
        '<p class="text-xs text-base-content/40 text-center py-4">正在构建发送给AI的提示词...</p>';
    ...
    try {
      ...
      if (result.error) {
        if (dom.presetPreviewContent)
          dom.presetPreviewContent.innerHTML = `<p class="text-xs text-error py-4">❌ ${escapeHtml(result.error)}</p>`;
        return;
      }
      ...
      if (dom.presetPreviewContent)
        dom.presetPreviewContent.innerHTML = `<p class="text-xs text-error py-4">❌ 预览失败: ${escapeHtml(err.message)}</p>`;
    }
  });
  ...
  dom.presetPreviewCopy?.addEventListener("click", async () => {
    ...
    const allText = Array.from(
      dom.presetPreviewContent.querySelectorAll(".mp-preset-preview-text"),
    )
  ...
  dom.runPresetBtn?.addEventListener("click", async () => {
    ...
    if (dom.runReplyContent)
      dom.runReplyContent.textContent = "正在调用记忆AI...";
  ...
  dom.runResultCopy?.addEventListener("click", async () => {
    const parts = [];
    if (dom.runThinkingContent?.textContent)
      parts.push(`[思维链]\n${dom.runThinkingContent.textContent}`);
    if (dom.runReplyContent?.textContent)
      parts.push(`[AI回复]\n${dom.runReplyContent.textContent}`);
    if (dom.runOperationsList?.textContent)
...
function renderRetrievalConfig() {
  ...
  if (dom.cfgAutoTrigger)
    dom.cfgAutoTrigger.checked = retrieval.auto_trigger !== false;
  if (dom.cfgChatHistoryCount)
    dom.cfgChatHistoryCount.value = retrieval.chat_history_count ?? 5;
  if (dom.cfgMaxSearchRounds)
    dom.cfgMaxSearchRounds.value = retrieval.max_search_rounds ?? 5;
  if (dom.cfgTimeout) dom.cfgTimeout.value = retrieval.timeout_ms ?? 60000;

  // 预设切换冷却
  const ps = currentConfig.preset_switch || {};
  if (dom.cfgCooldownRounds)
    dom.cfgCooldownRounds.value = ps.cooldown_rounds ?? 5;
}
...
function showMaintenanceStatus(msg, autoClearMs = 0) {
  if (dom.maintenanceStatus) {
    ...
    if (autoClearMs > 0) {
      setTimeout(() => {
        dom.maintenanceStatus.textContent = "";
      }, autoClearMs);
    }
  }
}
...
function renderDryRunFormatted(messages) {
  messages.forEach((msg, idx) => {
    const card = document.createElement("div");
    card.className =
      "rounded-md border border-base-content/10 overflow-hidden mb-2";
    ...
    pre.className =
      "mp-preset-preview-text text-xs font-mono whitespace-pre-wrap p-3 max-h-96 overflow-y-auto";
...
function renderDryRunRawJson(messages) {
  ...
  pre.style.cssText =
    "background: oklch(var(--bc) / 0.05); border: 1px solid oklch(var(--bc) / 0.1); max-height: 70vh;";
...
function renderRunResult(result) {
  ...
  if (result.thinking) {
    ...
    if (dom.runThinkingContent)
      dom.runThinkingContent.textContent = result.thinking;
  ...
  if (dom.runReplyContent)
    dom.runReplyContent.textContent = result.reply || "（无回复内容）";
...
function renderPresetSwitchSection(preset) {
  ...
  section.style.cssText =
    "border: 1px solid oklch(var(--bc) / 0.1); border-radius: 8px; padding: 12px;";
  ...
  // 插入到提示词标题之前
  const promptHeader =
    targetContainer.querySelector("#mp-prompt-list")?.previousElementSibling;
  if (promptHeader) {
...
function appendPresetSwitchEntryRow(container, presetName, description) {
  ...
  if (
    container.querySelector(
      `.mp-ps-entry-row[data-preset-name="${presetName}"]`,
    )
  )
    return;
  ...
  row.style.cssText =
    "background: oklch(var(--bc) / 0.03); border: 1px solid oklch(var(--bc) / 0.06);";
...
function exportPresets() {
  ...
  showStatus(
    `✅ 已导出 ${presets.length} 个预设 + ${injectionPrompts.length} 个注入条目`,
    3000,
  );
}
...
function importPresets() {
  ...
  input.addEventListener("change", async (e) => {
    ...
    try {
      ...
      // 确认导入
      const presetNames = importData.presets
        .map((p) => `${p.id}(${p.name})`)
        .join(", ");
      const injNames = importData.injection_prompts
        .map((p) => `${p.id}(${p.name})`)
        .join(", ");
      ...
      if (result && result.success) {
        await refreshPresets();
        showStatus(
          `✅ 导入成功: ${result.presetsCount} 个预设, ${result.injectionCount} 个注入条目`,
          4000,
        );
      } else {
