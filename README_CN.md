...
- **分层的记忆与上下文** — Data + hot / warm / cold 分层保存长期材料，一个收集上下文、检索记忆的工具（P1）在每轮回答前召回当前相关的片段；上下文清理做到文件读取级颗粒度、可逆，AI 也能自行放弃不再需要的已读文件；
- **核心行为可查看、可配置** — 角色、提示词、注入、记忆、召回路线、权限与插件都有对应的编辑或配置入口；
- **基于插件的扩展框架** — 核心功能以插件组织，经中间信息站传导，前端负责展示与操作；用户插件可用 JS、Python 或独立程序编写；
- **一套集成的 agent 工具链** — 在同一套记忆与上下文框架下提供文件、命令、浏览器集成、MCP、多窗口、审批与恢复；实际可用范围和效果取决于所选模式、配置、运行环境、模型与外部服务。
...
填入服务地址、API Key 和模型，保存后选择或导入一张角色卡，就能开始聊天。至少需要一个可用的 AI API；模型能力与费用取决于你绑定的服务。应用内置 [Wiki](site/wiki/getting-started/overview.md)，也可访问[在线版](https://beilusaiying.github.io/always-accompany/)。
...
- **👑 主要提示词条目可编辑**：角色定义、预设、INJ 条目、模式说明、记忆数据槽与工具说明，会按对应编辑器提供内容、顺序、开关、身份、注入位置或条件设置；框架安全包装和供应商特定的消息转换仍由代码管理；
...
把它们与 P1（一个收集上下文、检索记忆的工具）组合起来，就形成“向量 + 动态注入 + 记忆跟着当前任务走”的可配置流程，文件读取级上下文清理也在同一条链路中。实际召回与压缩效果仍取决于所选模式、配置、材料和模型。
...
<details>
<summary><strong>👑 主要提示词条目可编辑 — 默认可用，也能按自己的流程配置</strong></summary>

角色定义、预设、INJ 条目、模式说明、记忆数据槽、工具说明等主要提示词条目，可以在界面里编辑。每类条目可调整其编辑器实际提供的选项；框架安全包装和供应商特定的消息转换仍由代码管理：
...
**当前仓库已经具备的入口与实现**：Data + 三层记忆 · 上下文管理 · 自驱动 P1 / AI P1 · 提示词条目编辑与预设切换 · 模式记忆表 · 条件知识动态注入 · Live2D / 图片桌宠 · 屏幕感知与游戏陪伴 · 本地语音输入 · PPT 生成 · MCP · 多窗口 · VS Code 扩展桥 · 9 个平台 Bot · 23 个内置插件目录 · 用户插件宿主 · 回收 / 备份链 · 白盒诊断 · 多语言与主题。
...
always-accompany 不是为了把热门功能塞进同一个菜单——一开始只是作者自己想用 :)。它包含插件与框架体系，并提供多种界面语言；实际覆盖范围会随插件和翻译资源而变化。
