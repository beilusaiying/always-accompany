...
import { confirmI18n, console, geti18n, i18nElement, initTranslations, promptI18n } from '../../scripts/i18n.mjs'
import { createJsonEditor } from '../../scripts/jsonEditor.mjs'
import { getAllDefaultParts, getPartList, setDefaultPart, unlockAchievement, unsetDefaultPart } from '../../scripts/parts.mjs'
import { svgInliner } from '../../scripts/svgInliner.mjs'
import { applyTheme } from '../../scripts/theme.mjs'
import { showToast, showToastI18n } from '../../scripts/toast.mjs'

import { addServiceSourceFile, deleteServiceSourceFile, getConfigDisplay, getConfigTemplate, getServiceSourceFile, setServiceSourceFile } from './src/endpoints.mjs'
...
function highlightActiveFile() {
	...
	if (activeFile) {
		const activeItem = fileListContainer.querySelector(`.file-list-item[data-name="${activeFile}"]`)
		if (activeItem) activeItem.classList.add('active')
	}
}

/**
 * 确保加载一个文件（从URL、activeFile或列表第一个）。
 */
async function ensureFileLoaded() {
	...
	else if (activeFile && fileList.includes(activeFile))
		fileToLoad = activeFile
	else if (fileList.length)
		fileToLoad = fileList[0]

	if (fileToLoad) {
		if (activeFile !== fileToLoad)
			await loadEditor(fileToLoad)
	}
	else {
		activeFile = null
		updateEditorContent({})
...
async function loadGeneratorAddons(generatorName) {
	...
	try {
		...
		if (displayScript) {
			...
			// 立即触发一次 onJsonUpdate 以确保 generatorDisplay 能够获取到初始数据
			if (jsonEditor) {
				let initialData
				try { initialData = jsonEditor.get() || JSON.parse(jsonEditor.get().text) } catch (e) { initialData = {} }
				onJsonUpdate({
					data: initialData,
					containers: {
...
async function loadEditor(fileName) {
	...
	if (!generatorSelect.value) {
		...
		// 立即触发一次 onJsonUpdate 以确保 generatorDisplay 能够获取到初始数据
		let initialData
		try { initialData = jsonEditor.get() || JSON.parse(jsonEditor.get().text) } catch (e) { initialData = data.config }
		
		// 确保 onJsonUpdate 存在且是函数
		if (typeof onJsonUpdate === 'function') {
			onJsonUpdate({
				data: initialData,
				containers: {
					generatorDisplay: generatorDisplayContainer,
...
async function saveFile() {
	...
	try {
		...
		// 刷新文件列表以确保同步，但保持当前选中状态
		// await fetchFileList() // 移除此行，避免不必要的列表刷新导致状态重置
	}
	catch (error) {
