...
一套三层递归记忆（日→月→年归档，纯 JSON，260 年容量）+ 自驱动召回引擎（纯算法管线，零 LLM 零网络，毫秒级：BLQ 质量评分 + NB300 向量验证 + WordNet 交叉确认 + BM25 混合排序 + 时间衰减，三层过滤自动淘汰噪声词）+ 分层上下文清理（清掉的只是不再发送，原文留着随时恢复）。这三件事咬合在一起，让 AI 不再受上下文窗口限制地记住你说过的每一句话。在此基础上，我们做了聊天/角色扮演、IDE 编程、工作模式（含 AI 做 PPT）、Live2D 桌宠（屏幕感知+游戏陪伴）、语音输入、Discord Bot、MCP 外部工具接入——所有入口共享同一套记忆，换个窗口 AI 依然认识你。
...
<details>
<summary><strong>🔬 自驱动 P1 — 已上线的零 LLM 召回引擎</strong></summary>

AI P1 每轮要发 API 请求——有延迟、有成本、离线不能用。我们写了一条完整的纯算法管线（五节点独立服务架构，~12,000 行），已在生产环境运行：毫秒级、零网络、三层过滤自动淘汰噪声。
...
**已完成**：三层记忆 · 压缩系统 · P1 检索 · **自驱动 P1 召回引擎（BLQ+NB300+WordNet 三层过滤，独立服务架构，集群懒启动）** · 提示词引擎 · 预设自动切换 · 记忆表格 · 世界书动态注入 · Live2D 桌宠 · 游戏陪伴 · 语音输入 · AI 做 PPT · MCP · 多窗口并行 · VSCode 扩展桥 · Discord Bot · 22 插件 · 回收站与备份回档 · 全栈诊断 · 多语言 · 五层隔离（窗口/角色卡/用户/chatId/模式）

**近期计划**：更多 Bot 平台 · 插件生态 · TTS / 文生图 · AI 游戏引擎（era 系血统，数值确定性代码+LLM 叙事+符号渲染）· 直播模式
