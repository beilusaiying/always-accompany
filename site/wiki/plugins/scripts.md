...
beilu-ejs（连同 beilu-mvu 变量联动）**默认关闭**，需要在面板显式开启（opt-in）。开启后的渲染范围也有硬性隔离：**只有标记为 `ephemeral`（注入型临时内容）的文本会参与 EJS 渲染，真实聊天正文永远不会被当作模板执行**。世界书里的 EJS 条目要生效需同时满足三个条件：条目存在 + 用户开关开启 + 该世界书是当前角色卡的主世界书——三缺一不渲染。
