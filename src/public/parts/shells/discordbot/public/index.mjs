...
// ---- 可视化配置表单元素 ----
const cfgOwnerId = document.getElementById("cfg-owner-id");
...
function populateVisualConfig(cfg) {
  cfgOwnerId.value = cfg.OwnerUserId || "";
  cfgOwner.value = cfg.OwnerUserName || "";
  // T15-4：删前端硬编码默认副本 ?? 20 / ?? 30；值由后端 getBotConfigTemplate 单源下发（后端
  //   GetSimpleBotConfigTemplate: MaxMessageDepth=BOT_DEFAULT_MAX_MESSAGE_DEPTH / MaxFetchCount=30）。
  //   空对象（清空表单态 :203）时显示空，符合"无 bot 选中"语义。
  cfgMaxDepth.value = cfg.MaxMessageDepth ?? "";
  cfgMaxFetch.value = cfg.MaxFetchCount ?? "";
...
function readVisualConfig() {
  return {
    OwnerUserId: cfgOwnerId.value.trim(),
    OwnerUserName: cfgOwner.value.trim(),
    // T15-4：删前端硬编码默认副本 || 20 / || 30；空输入发 undefined（JSON 序列化省略），
    //   后端消费侧单源兜底（default_interface/main.mjs:134 `|| BOT_DEFAULT_MAX_MESSAGE_DEPTH`、
    //   :136 `|| Math.max(...)`），前端不再持有默认数字副本。
    MaxMessageDepth: parseInt(cfgMaxDepth.value) || undefined,
    MaxFetchCount: parseInt(cfgMaxFetch.value) || undefined,
...
function renderLogEntry(entry) {
  ...
  if (entry.type === "ai" && entry.thinking) {
    const thinkingId = `think-${String(entry.id).replace(/[^a-zA-Z0-9_-]/g, '_')}`;
    html += `<div class="msg-thinking-toggle" data-toggle-target="${escapeHtml(thinkingId)}">💭 展开思维链</div>
    <div id="${escapeHtml(thinkingId)}" class="msg-thinking hidden">${escapeHtml(entry.thinking)}</div>`;
  }

  // AI 回复的原始完整内容展开
  if (entry.type === "ai" && entry.fullContent) {
    const fullId = `full-${String(entry.id).replace(/[^a-zA-Z0-9_-]/g, '_')}`;
    html += `<div class="msg-thinking-toggle" data-toggle-target="${escapeHtml(fullId)}">📄 展开原始内容</div>
    <div id="${escapeHtml(fullId)}" class="msg-thinking hidden">${escapeHtml(entry.fullContent)}</div>`;
  }
...
function escapeHtml(str) {
  // T7c：原 DOM textContent 法不转义引号，而 :633/:634/:640/:641 在属性上下文（id/data-*）调用本函数，
  // 引号可致属性逃逸。改为 5 字符全转义（与 chat 壳权威版/directory_listing.mjs 同型）。
  return String(str)
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;")
...
async function init() {
  ...
  // 思维链/原始内容展开折叠（事件委托，替代 inline onclick 防 XSS）
  messageLogList?.addEventListener("click", (e) => {
    const toggle = e.target.closest("[data-toggle-target]");
    if (toggle) { const t = document.getElementById(toggle.dataset.toggleTarget); if (t) t.classList.toggle("hidden"); }
  });
  ...
  for (const el of [cfgOwnerId, cfgOwner, cfgMaxDepth, cfgMaxFetch, cfgTriggerChannels]) {
