 ...
 *
 * [2026-08-06 框架化] 实现体从 yonban/core/functions/prompt/mvu/main.mjs 迁回本 part 目录（凛倾拍板
 *   "改成框架,而不是耦合核心"，方向与当年 T3d"随组集中"相反）。改动仅 5 处 import 路径
 *   （parts_loader/wbStub/safeJsonIO 层级 5 级改 4 级、info 改同目录、auth 指 yonban 绝对层级），其余逐字保持。
 *   core 侧 functions/prompt/index.mjs facade 改指本文件（同一物理 URL → ESM 单实例，无双实例风险）。
 *   mvu 数据经 setDefaultPart(username,"plugins","beilu-mvu") 按 part 名字符串 keyed 落盘（非 import.meta.url），无路径锚需求。
 *
...
import { authenticate } from "../../../../yonban/core/functions/security/auth.mjs"; // A2-3：HTTP 端点鉴权中间件（未认证→401），与全站 router.get/post(path, authenticate, handler) 同型（与 beilu-preset 同写法）
...
async function initFromWorldBook(arg) {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        if (parsed && typeof parsed === "object") {
          ...
          // [0806 可观测] MVU 变量初始化成功 → 白盒留痕。用户看到"变量管理器空白"时，
          //   这条能区分"世界书里根本没 [InitVar]"与"读到了但解析失败"。
          try { wbT(null, "mvu:initvar", "init_ok", { charName, entries: entries.length, rootKeys: Object.keys(parsed).slice(0, 20) }); } catch { /* 白盒不可用不阻断 */ }
    ...
    // [0806 可观测] 走到这里=世界书拿到了但没有任何 [InitVar] 条目命中。
    //   这是"变量管理器空白"最常见的原因，原来完全静默（返回 null 无任何痕迹），
    //   用户与开发者都无法区分"没绑世界书/绑了但没 InitVar/读取报错"三种情况。
    try {
      wbT(null, "mvu:initvar", "no_initvar_entry", {
        charName, entries: entries.length,
        hint: entries.length === 0 ? "世界书为空或未绑定到该角色" : "世界书有条目但无 [InitVar] 标记",
      });
    } catch { /* 白盒不可用不阻断 */ }
  } catch (err) {
  ...
  interfaces: {
    ...
    chat: {
      ...
      ReplyHandler: (result, request) => {
        ...
        //   改取 content_for_show（此刻=含 thinking 原文+哨兵已追加，变量标签剥离在其后才发生，编辑态保留标签）；
