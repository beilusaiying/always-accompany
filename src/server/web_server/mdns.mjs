...
export async function initMdns(port, protocol, config) {
	const mdns_config = {
		name: port === 1315 ? 'beilu-sandbox' : 'beilu',
		port,
		type: protocol,
		txt: {
			description: geti18n('beiluConsole.server.mdns.description'),
		},
		...config,
	}
	...
	mdns.advertise().catch(async error => { // 不应await此操作，会阻塞服务器
		console.errorI18n('beiluConsole.server.mdns.failed', { error })
		mdns.stop()
	*/
	const { Bonjour } = await import('npm:bonjour-service')
	const instance = new Bonjour({}, error => {
		console.errorI18n('beiluConsole.server.mdns.bonjourFailed', { error })
	})
