...
def prepare_recall_context(input_text, chat_history, config=None):
    ...
    config = config or {}
    context_messages = _finite_int(config.get("contextMessages"), 0, 5, 3)
    ...
    selected = []
    # 0 是合法的“关闭上下文”，不可让 [-0:] 反转成全量历史。
    selected_candidates = user_candidates[-context_messages:] if context_messages > 0 else []
