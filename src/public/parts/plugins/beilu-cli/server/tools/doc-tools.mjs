/**
 * doc-tools.mjs — xlsx 公式编辑（从 YonBan doc-tools.ts 1:1 移植）
 */
import * as fs from "node:fs";
import * as path from "node:path";
import { resolveWorkspacePath, assertWritable, decodeXmlEntities, mapXlsxSheets } from "../infra.mjs";
...
function _setCellFormula(xml, cell, formula) {
...
export async function editXlsx(params, checkpoint) {
  ...
  if (path.extname(absPath).toLowerCase() !== ".xlsx") throw new Error("edit_xlsx 仅支持 .xlsx");
  ...
  const sheetMap = mapXlsxSheets(wbXml, relsXml);
  ...
  try {
    ...
    throw new Error(`定点改写后文件校验失败（未写盘，原文件未动）：${e?.message || String(e)}`);
  }

  fs.writeFileSync(absPath, outBuf);
  return {
    path: filePath, edited: applied, count: applied.length,
    message: `已定点写入 ${applied.length} 个单元格公式。Excel 打开会自动重算。`,
  };
}
