...
> 版本: v1.5
> 日期: 2026-03-08
> 状态: 1.5 版 — 前端大幅优化 + DeepSeek 兼容 + 多项 Bug 修复 + Markdown 渲染器增强
...
| 能力             | 说明                                                                           |
| ---------------- | ------------------------------------------------------------------------------ |
| **IDE**          | VSCode 风格三栏布局，集成文件管理与代码编辑，通用放大编辑器                    |
| **AI 引擎**      | 多 AI 协作架构（聊天 AI + 6 个辅助 AI），commanderMode 消息控制，DeepSeek 兼容 |
| **原创记忆算法** | 4 层分级记忆系统，参照人类海马体结构与记忆曲线原理设计                         |
| **聊天系统**     | 兼容 SillyTavern 预设 / 角色卡 / 世界书格式，背景图系统，Markdown 渲染增强     |
...
| **beilu-browser**   | 浏览器页面感知（被动+按需双通道）               | `plugins/beilu-browser/main.mjs`   |

### 3.2 Shell 体系

| Shell                   | 功能                                                                                                                       | 路径                          |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------- | ----------------------------- |
| **beilu-home**          | 管理首页（角色卡 / 预设 / 世界书 / 记忆 / 人设 / 系统查看器 / 调试 / 日志），支持多语言（中/英/日/繁），字体滑块，横栏切换 | `shells/beilu-home/`          |
| **beilu-chat**          | 聊天界面（三栏 IDE 布局、记忆浏览器、文件操作、图片收发、背景图系统、通用放大编辑器、用户人设编辑、世界书条目编辑器）      | `shells/beilu-chat/`          |
| **chat**                | Fount 后端聊天逻辑（含 deleteMessagesRange）                                                                               | `shells/chat/`                |
| **serviceSourceManage** | 服务源管理                                                                                                                 | `shells/serviceSourceManage/` |
| **proxy**               | AI 代理（commanderMode 版，DeepSeek 兼容）                                                                                 | `shells/proxy/`               |
| **install**             | 角色卡导入处理                                                                                                             | `shells/install/`             |
| **discordbot**          | Discord Bot（跨平台 AI 服务，可视化管理面板 + 实时消息日志）                                                               | `shells/discordbot/`          |
...
| ID  | 名称             | 触发              | 用途                                    |
| --- | ---------------- | ----------------- | --------------------------------------- |
| P1  | 检索 AI          | auto_on_message   | 多轮深度检索（最多 3 轮）+ 智能预设切换 |
...
### v1.5（2026-03-08）— 前端大幅优化 + DeepSeek 兼容 + Bug 修复

#### 前端 CSS 优化大版本（21项）

| 类型    | 内容                                                                                                                                                                                                                               |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0 CSS  | 字体渲染优化（antialiased） / 对话头像 50px + 琥珀色描边 / 消息排版优化（line-height:1.53） / 消息按钮透明度 / Home 间距优化 / 角色卡放大（3:4 长方形）                                                                            |
| P1 功能 | 选项卡名称修改 / 字体大小 range 滑块 / **通用放大编辑器组件** / **背景图系统**（4 种缩放模式 + 透明度/模糊滑块） / **Chat 用户人设编辑** / Home textarea 放大 / **Home 左侧导航横栏切换** / Chat 世界书条目编辑器（HTML+CSS 框架） |

#### DeepSeek API 兼容性修复（9项）

参照 SillyTavern `sendDeepSeekRequest` 实现，修复 proxy 层缺失的 DeepSeek 特化处理：

- `patchBodyForDeepSeek`：黑名单参数删除 + max_tokens 裁剪 + top_p 兜底 + 空 tools 清理
- `mergeConsecutiveRoles()`：合并连续同角色消息
- 尾部 assistant 转 system + 追加 user 触发消息
- URL 前后空格 trim + 协议补全
- API 源热更新：保存/切换时自动同步角色卡绑定

#### Bug 修复

| 修复项                        | 说明                                                                                  |
| ----------------------------- | ------------------------------------------------------------------------------------- |
| 新建角色卡死                  | 删除 `new/index.mjs` 中 2 处 `history.replaceState`，修复导航阻断                     |
| 角色描述加载                  | description fallback（char-data API → personality → system_prompt）+ placeholder 更新 |
| Markdown 渲染器               | 添加 `rehype-raw` 插件，修复 HTML 标签内内容不渲染                                    |
| parsePresetSwitchTag 截断兜底 | 截断回复也能正确解析 presetSwitch 标签                                                |
| SSE 非 JSON data 过滤         | `data: [DONE]` 等非 JSON 行不再报错                                                   |
| handleNewChat 角色存在性检查  | 404 时 alert 提示                                                                     |
| 浏览器感知行为调整            | autoInject 默认 false，禁止自动注入                                                   |
| setting_loader mkdirSync      | 新用户首次访问 ENOENT 修复                                                            |

### v1.4（2026-02-27）— 智能检索 + Discord Bot + 浏览器感知

- **BM25 + 正则搜索双引擎**：P1 检索从 5 轮优化至最多 3 轮（速度 +40%、成本 -40%）
- **Discord Bot**：跨平台 AI 服务，可视化管理面板 + 实时消息日志
- **浏览器页面感知**：被动式 + 按需式双通道架构，油猴脚本监听 DOM 变化
- **全栈诊断框架**：模块级开关 + Console 拦截 + 一键打包日志导出
- **智能预设切换**：P1 实时分析对话意图，自动切换提示词预设

### v1.3（2026-02-23）— 智能预设切换

- P1 检索 AI 实时分析上下文自动切换提示词预设
- `{{presetList}}` 宏 + `<presetSwitch>` 指令
- 冷却防振荡 + 手动一键切换

### v1.2（2026-02-22）— 渲染引擎 + 正则美化

- iframe 沙箱渲染 + ST API 兼容注入
- 正则引擎对齐酒馆回调模式
- 渲染器开关 / 深度设置 / 代码折叠 / 流式渲染

### v1.1（2026-02-21）— 预设解耦 + i18n

- 预设三级加载（用户 > 模板 > 骨架）
- 导出安全清洗
- beilu-home 多语言支持（中/英/日/繁）
...
*此文档综合了 beilu-always accompany 项目截至 2026-03-08 的所有工作日志、设计文档和代码分析。如需详细了解某个模块，请参考对应的工作日志文件。*
