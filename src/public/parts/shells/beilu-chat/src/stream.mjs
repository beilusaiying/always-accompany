/**
 * beilu-chat 流式工具层 — 管「流式预览节流」「文本 diff 切片」「工具调用隐藏/内联执行」「假流式节奏推流」。
 * 不管 WS 广播（那是 broadcast.mjs 的事）、不管生成调度（那是 generation.mjs 的事）。
 *
 * 链路：broadcast.StreamManager.create → createBufferedSyncPreviewUpdater(33ms 节流) → generateDiff → 返回 slices
 *       → broadcastChatEvent(stream_update, {slices}) → 前端 applySlice(append/rewrite_tail/set_files)
 * 影响：无持久化副作用、无全局状态 — 纯工具函数层
 * 相交：← broadcast.mjs(StreamManager 内部消费)·proxy/main.mjs(replyPreviewUpdater 链)
 *       → 前端 websocket.mjs handleStreamUpdate(消费 slices)
 */
 ...
 * 使用定时节流（~30fps）替代微任务调度，确保 SSE 同步循环中的多次调用
 * 不会全部被合并为一次更新，从而实现前端逐字/逐段流式效果。
 *
 * 首次调用零延迟（首 token 立即送达），后续在节流窗口内合并，
 * 窗口结束时补发最新状态，不丢失任何中间更新。
 *
...
export function createBufferedSyncPreviewUpdater(asyncPreviewUpdater) {
	...
	const THROTTLE_MS = 33 // ~30fps，与前端 rAF 渲染频率匹配

	/**
	 * 将当前缓存的最新 reply 排入异步更新队列。
	 */
	function drain() {
		...
		pending = pending.then(() =>
			Promise.resolve(asyncPreviewUpdater(reply)).catch(e => { console.warn("[stream] 预览更新失败:", e?.message || e); })
		)
	}

	return function update(reply) {
		// 浅拷贝：content 是字符串（值拷贝），files 只 push 不删改
		lastReply = {
			content: reply.content,
			content_for_show: reply.content_for_show,
			files: reply.files,
		}
		if (throttleTimer === null) {
			drain() // 首次调用立即 drain（零延迟首 token）
			throttleTimer = setTimeout(() => {
				throttleTimer = null
				if (lastReply) drain() // 窗口结束时补发最新状态
			}, THROTTLE_MS)
		}
	}
}
...
/**
 * 定义工具调用隐藏器 — 在流式预览中将 start/end 标签对包裹的内容替换为本地化占位卡片。
 *
 * 链路：角色模板 GetReply → replyPreviewUpdater 链（装饰器模式，洋葱式 next 调用）→ 本函数返回的 updater
 *       → 正则替换工具标签为 HTML 卡片/markdown 占位 → 传给下一层 updater
 * 约束：pattern 用 RegExp('g') 全局匹配后必须 reset lastIndex（已做），否则连续调用会跳匹配
 *
 * @param {{start: string|RegExp, end: string|RegExp}[]} toolPairs - 工具对数组（start/end 标签）
 * @returns {import('../../chat/decl/chatLog.ts').CharReplyPreviewUpdater_t} 回复预览更新器（装饰器）
 */
...
<div class="tool-call-placeholder card bg-base-100 shadow-xl">
	<div class="card-body">
	${args.supported_functions.beilu_i18nkeys ?
...
/**
 * 定义内联工具使用处理器 — 流式预览中检测已闭合的 start/end 标签对，执行 exec 回调并将结果替换回内容。
 * 支持异步执行+缓存（同一匹配只执行一次），未闭合的标签对隐藏不显示。
 *
 * 链路：角色模板 GetReply → replyPreviewUpdater 链 → 本函数返回的 updater
 *       → matchAll 找闭合对 → exec(content) → 替换结果 → 隐藏未闭合尾部 → 传给 next
 * 影响：通过 args.extension.streamInlineToolsResults 跨帧缓存执行结果（按 id 分类）
 *
 * @param {Array<[string, string|RegExp, string|RegExp, (content: string) => string|Promise<string>]>} toolDefs - 工具定义数组 [id, start, end, exec]
 * @returns {import('../../chat/decl/chatLog.ts').CharReplyPreviewUpdater_t} 回复预览更新器（装饰器）
 */
...
/**
 * 生成两个消息对象之间的差异切片（文本+文件），供前端 applySlice 消费。
 *
 * 链路：StreamManager.create 内 syncUpdate → 本函数 → 返回 slices → broadcastChatEvent(stream_update)
 * 约束：优先取 content_for_show（已剥 <emotion> 等标签），无则退回 content；
 *       三种文本 slice: append(纯追加,最常见) / rewrite_tail(内容重写) / 空(完全相等)；
 *       文件 slice: set_files(文件列表长度或名称变化)
 *
 * @param {object} oldMessage - 旧消息对象（含 content/content_for_show/files）
 * @param {object} newMessage - 新消息对象
 * @returns {Array<{type: 'append'|'rewrite_tail'|'set_files', [key: string]: any}>} 差异切片数组
 */
 ...
 *
 * 链路：createLineBasedIncrementalStream → 本函数 → 按 setTimeout 节奏调度 onChunk
 *
 ...
 *
 * 链路：createBufferedLineBasedStream → 本函数 → createPacedFakeStream(内部节奏) → onChunk
 *
 * @param {object} options - 配置。
...
export function createLineBasedIncrementalStream(options) {
	...
	function update(reply) {
		...
		const { files } = reply
		pending = pending.then(() => processUpdate(text, files)).catch(e => { console.warn("[stream] processUpdate error:", e?.message); })
		return pending
	}
 ...
 * 符合 beilu 默认的「不每次 await 上游更新、由 buffer 合并后再更新界面」的用法。
 *
