...
systemNavItems.forEach((btn) => {
  btn.addEventListener("click", () => {
    ...
    dynamicTitle.textContent =
      t(systemSectionTitleKeys[sectionId]) || sectionId;
  });
});
...
document.getElementById("api-warning-goto")?.addEventListener("click", () => {
  switchTab("system");
  // 确保 AI 服务源面板激活
  const apiNav = document.querySelector(
    '.beilu-system-nav-item[data-sys-section="api"]',
  );
...
function initBeiluEyePanel() {
  ...
  async function refreshEyeStatus() {
    try {
      ...
      if (!res.ok) {
        if (statusBadge)
          statusBadge.textContent = t("sys.plugins.eye.notLoaded");
        return;
      }
      const data = await res.json();
      if (statusBadge) {
        const labelKeys = {
          stopped: "sys.plugins.eye.stopped",
          installing: "sys.plugins.eye.installing",
          starting: "sys.plugins.eye.starting",
          running: "sys.plugins.eye.running",
          error: "sys.plugins.eye.error",
        };
        statusBadge.textContent = labelKeys[data.electronStatus]
          ? t(labelKeys[data.electronStatus])
          : data.electronStatus;
        statusBadge.className = "badge badge-sm badge-outline";
        if (data.electronStatus === "running")
          statusBadge.classList.add("badge-success");
        else if (data.electronStatus === "error")
  ...
  restartBtn?.addEventListener("click", () =>
    sendEyeAction("restart-electron"),
  );
}
...
function initDarkMode() {
  ...
  // 更新描述文字
  const desc = toggle
    .closest(".beilu-settings-item")
    ?.querySelector(".text-xs");
  if (desc)
    desc.textContent = isDark
      ? t("user.theme.darkMode.on")
      : t("user.theme.darkMode.off");
...
function initFontSize() {
  const fontSizeRange = document.getElementById("user-font-size");
  const fontSizeLabel = document.getElementById("user-font-size-label");
  if (!fontSizeRange) return;

  // 旧版迁移：如果存的是字符串名称，转换为数值
  const NAME_TO_PX = {
    xsmall: 11,
    small: 12,
    medium: 14,
  ...
  let saved = localStorage.getItem("beilu-font-size") || "14";
  if (NAME_TO_PX[saved] !== undefined) {
    saved = String(NAME_TO_PX[saved]);
    localStorage.setItem("beilu-font-size", saved);
  }

  const px = Math.max(12, Math.min(24, parseInt(saved, 10) || 14));
  fontSizeRange.value = px;
  if (fontSizeLabel) fontSizeLabel.textContent = `${px}px`;
  applyFontSize(px);

  fontSizeRange.addEventListener("input", () => {
    const size = parseInt(fontSizeRange.value, 10);
    localStorage.setItem("beilu-font-size", String(size));
    if (fontSizeLabel) fontSizeLabel.textContent = `${size}px`;
    applyFontSize(size);
  });

  // < > 按钮：精确增减 1px
  const decBtn = document.getElementById("user-font-size-dec");
  const incBtn = document.getElementById("user-font-size-inc");

  if (decBtn) {
    decBtn.addEventListener("click", () => {
      const cur = parseInt(fontSizeRange.value, 10);
      const next = Math.max(12, cur - 1);
      fontSizeRange.value = next;
      localStorage.setItem("beilu-font-size", String(next));
...
function initNavLayout() {
  const toggle = document.getElementById("user-nav-layout");
  if (!toggle) return;

  const layouts = document.querySelectorAll(".beilu-usage-layout");
  const saved = localStorage.getItem("beilu-nav-layout");
  const isHorizontal = saved === "horizontal";

  toggle.checked = isHorizontal;
  if (isHorizontal) {
    layouts.forEach((l) => l.classList.add("nav-horizontal"));
  }

  toggle.addEventListener("change", () => {
    const horizontal = toggle.checked;
    localStorage.setItem(
      "beilu-nav-layout",
      horizontal ? "horizontal" : "vertical",
    );
    layouts.forEach((l) => {
      l.classList.toggle("nav-horizontal", horizontal);
    });
  });
}
      ...
      copyBtn.addEventListener("click", () => {
        ...
        navigator.clipboard
          ...
          .then(() => {
            ...
            setTimeout(() => {
              copyBtn.textContent = t("user.remote.copy");
            }, 2000);
          })
          .catch(() => {
            // fallback
...
function initMobileAdaptation() {
  ...
  const allNavItems = document.querySelectorAll(
    ".beilu-usage-nav-item, .beilu-system-nav-item, .beilu-user-nav-item",
  );
  ...
  function handleResize() {
    if (window.innerWidth > MOBILE_BREAKPOINT) {
      ...
      // 手机：默认显示导航
      const hasActive = [...layouts].some(
        (l) =>
          l.classList.contains("mobile-content-active") ||
          l.classList.contains("mobile-nav-active"),
      );
      if (!hasActive) {
