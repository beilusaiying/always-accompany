...
async function handleProtocol() {
	...
	if (!protocol || !protocol.startsWith('beilu://')) {
	...
	if (!authResponse.ok) {
		// redirect 只传同源路径（pathname+search+hash），不传绝对 URL——login 侧开放重定向围栏
		//   只放行以 / 开头的同源路径（login/index.mjs:redirectAfterLogin），绝对 URL 会被拒落默认壳。
		window.location.href = `/login?redirect=${encodeURIComponent(location.pathname + location.search + location.hash)}`
		return
	}
