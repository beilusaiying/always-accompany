 ...
 */
function getAllEnabledEntries(currentCharId = "", currentCharName = "") {
  const allEntries = [];
  for (const [name, wb] of Object.entries(configData.worldbooks)) {
    // boundCharName 仅用于导入/导出打包，不影响注入判断
    // 所有世界书统一看 enabled 字段决定是否注入
    ...
    try {
      ...
      if (saved?.worldbooks) {
        ...
        for (const [name, wb] of Object.entries(configData.worldbooks)) {
          ...
          // boundCharName 仅用于导入/导出打包，不影响注入
          // 保留用户原始 enabled 状态，不做强制开关迁移
          // 数据迁移：将旧的 ST 格式 entries 转换为 beilu 内部格式
          if (!wb.entries) continue;
          const entriesValues = Array.isArray(wb.entries)
            ? wb.entries
            : Object.values(wb.entries);
          const needsMigration =
            Array.isArray(wb.entries) ||
            (entriesValues.length > 0 &&
              entriesValues[0].uid === undefined &&
              entriesValues[0].id !== undefined);
          if (needsMigration) {
        ...
        const activeEntryCount = getActiveEntries()
          ? Object.keys(getActiveEntries()).length
          : 0;
        // 统计各模式分布
        const allEntries = Object.values(configData.worldbooks).flatMap((wb) =>
          wb.entries ? Object.values(wb.entries) : [],
        );
        ...
        diag.log(
          `初始化完成: ${count} 个世界书, 激活: "${active}" (${activeEntryCount} 条目), 模式分布: 常驻=${modeStats.constant} 正则=${modeStats.regex} 动态=${modeStats.dynamic}`,
        );
        console.log(
          `[beilu-worldbook] 已恢复 ${count} 个世界书, 激活: "${active}" (${activeEntryCount} 条目)`,
        );
      } else {
        ...
        try {
          ...
          if (!wb) {
            const queryDesc = charName
              ...
              : `名称="${bookName}"`;
            console.warn(
              `[beilu-worldbook] lorebook/entries: 未找到世界书 (${queryDesc})，可用: [${Object.keys(configData.worldbooks).join(", ")}]`,
            );
  ...
  interfaces: {
    config: {
      ...
      SetData: async (data) => {
        ...
        if (data.delete_worldbook) {
          ...
          if (configData.worldbooks[name]) {
            ...
            if (name === configData.active_worldbook) {
              ...
              configData.active_worldbook =
                remaining.length > 0 ? remaining[0] : "";
              console.log(
                `[beilu-worldbook] 已自动切换到: "${configData.active_worldbook || "(无)"}"`,
              );
            }
          }
        }
        ...
        if (data.rename_worldbook) {
          ...
          if (
            configData.worldbooks[old_name] &&
            !configData.worldbooks[new_name]
          ) {
        ...
        if (data.bind_worldbook) {
          ...
          if (configData.worldbooks[name]) {
            ...
            console.log(
              `[beilu-worldbook] 世界书 "${name}" ${charName ? "已绑定到角色: " + charName : "已解除角色绑定"}`,
            );
          }
        }
    ...
    chat: {
      ...
      GetPrompt: (arg) => {
        ...
        // ★ DIAG: 输出所有世界书的状态，帮助定位"额外世界书未注入"
        console.log(
          `[beilu-worldbook][DIAG] GetPrompt called: charId="${charId}", charName="${charName}", worldbooks:`,
          Object.entries(configData.worldbooks)
            .map(
              ([n, wb]) =>
        ...
        let entryArray = getAllEnabledEntries(charId, charName);

        // ★ DIAG: 输出 getAllEnabledEntries 结果
        console.log(
          `[beilu-worldbook][DIAG] getAllEnabledEntries("${charId}", "${charName}") → ${entryArray.length} 条目`,
        );
        ...
        // ★ DIAG: 详细输出激活结果
        console.log(
          `[beilu-worldbook][DIAG] GetPrompt 激活结果: ${activated.length} 条`,
          activated
            .map(
              (e) =>
                `"${e.comment || e.uid}"(${(e.content || "").length}字符, mode=${e.activationMode || "?"})`,
            )
            .join(", "),
        );
       ...
       *
       * 司令员模式下（beilu-preset commander_mode=true），跳过直接注入。
       * beilu-preset 会从 extension 中读取同样的数据，在 Round 2 中统一处理。
       * 如果这里也注入，会导致重复注入（内容出现两次）。
       */
