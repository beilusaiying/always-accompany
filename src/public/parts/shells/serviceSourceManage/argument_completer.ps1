# PowerShell 参数补全脚本，用于 beilu 的 'serviceSourceManage' shell。
#
# 使用方法:
#   beilu run shells <username> serviceSourceManage <action> [sourceName]
#
# 支持的 Action:
#   - list: 列出所有可用的 AI 源。
		...
		1 {
			...
			switch ($action) {
				{ $_ -in "delete", "get", "set", "set-default" } {
					# 对于这些需要指定已存在源的命令，调用 beilu 的内部函数来获取 AI 源列表，并进行补全。
					Get-BeiluPartList -parttype "serviceSources/AI" -Username $Username | Where-Object { $_.StartsWith($WordToComplete) }
					break
				}
				# "list" 命令没有参数，因此不提供补全。
