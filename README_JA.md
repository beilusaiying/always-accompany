...
- **独自の記憶とコンテキストのフレームワーク** — Data + hot / warm / cold の階層で長期素材を保存し、コンテキストを収集し記憶を検索するツール（P1）が毎回の回答前に現在関連する断片をリコールします；コンテキストのクリーンアップはファイル読み取り単位の粒度で可逆に行え、AI 自身がもう不要になった既読ファイルを手放すこともできます（作者の環境では課金基準の実測でキャッシュ効率は約 70–80%、承諾値ではありません）；
- **すべての内容が編集可能** — キャラクター、プロンプト、注入、記憶、リコール経路、権限とプラグインはどれもブラックボックスではなく、どの層を変えたくても入口があります；
- **高い拡張性を持つフレームワーク** — コア機能はプラグインとして構成され、中間の情報ステーションを経て伝達され、フロントエンドは表示と操作だけを担います；ユーザープラグインは JS、Python、あるいは独立したプログラムで書けます；
- **agent が持つべき全機能** — ファイル、コマンド、ブラウザ、MCP、複数ウィンドウ、承認と復元が揃い、しかも同じ一つの記憶とコンテキストのフレームワークを共有します；これは大規模プロジェクトをやり遂げるために生まれ、その核心はまさに、限られたアテンションを肝心なところに使うことです。
...
サービスのアドレス、API Key、モデルを入力し、保存してからキャラクターカードを選ぶかインポートすれば、チャットを始められます。少なくとも一つの使える AI API が必要です；モデルの能力と費用は、あなたが紐付けたサービス次第です。アプリには[完全な Wiki](site/wiki/getting-started/overview.md) が内蔵されており、[オンライン版](https://beilusaiying.github.io/always-accompany/)にもアクセスできます。
...
- **👑 すべてのプロンプトが編集可能**：内容、順序、オン / オフ、system / user / assistant の身元、注入位置と条件をすべて調整できます；
...
けれど、それらを組み合わせ、さらに P1（コンテキストを収集し記憶を検索するツール）を加えると、「ベクトル + 動的注入 + 記憶が現在のタスクに追従する」天然の生態系が出来上がります——高アテンション・高情報密度の記憶庫です；そこに、私たちがファイル単位まで実現した圧縮を組み合わせれば、鎖全体が完成します。
...
<details>
<summary><strong>👑 すべてのプロンプトが編集可能 — デフォルトのまま使え、自分の AI へと作り替え続けることもできる</strong></summary>

キャラクター設定、システムルール、モード説明、記憶データスロット、ツール教示などのプロンプト項目は、すべて画面で編集できます。一件ごとに次を調整できます：
...
**現在のリポジトリにすでに備わっている入口と実装**：Data + 三層記憶 · コンテキスト管理 · 自律駆動 P1 / AI P1 · 全プロンプト編集とプリセット切り替え · モード記憶テーブル · 条件付き知識の動的注入 · Live2D / 画像デスクトップペット · 画面認識とゲーム同伴 · ローカル音声入力 · PPT 生成 · MCP · 複数ウィンドウ · VS Code 拡張ブリッジ · 9 プラットフォームの Bot · 23 個の内蔵プラグインディレクトリ · ユーザープラグインホスト · 回収 / バックアップチェーン · ホワイトボックス診断 · 多言語とテーマ。
...
always-accompany は、流行りの機能を同じメニューに詰め込むために作られたのではありません——最初はただ、作者自身が使いたかっただけです :)。もちろん、完全なプラグインとフレームワークの体系を確かに備えており、複数の言語にも対応しています。
