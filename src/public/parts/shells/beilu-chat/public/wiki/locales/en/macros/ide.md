...
> Retired: `{{ide_tools}}` (tool instructions are no longer generated by code — all tool documentation lives in editable injection entries such as INJ-2 / INJ-2-code, editable in the memory preset panel); `{{ide_tool_results}}` (tool execution results enter the conversation as real messages, not via a macro). Any leftover occurrences of these two macros in entries are replaced with an empty string.
