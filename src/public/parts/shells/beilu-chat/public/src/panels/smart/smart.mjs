...
import { createVisibilityPoller } from "../../shared/state/windowRuntime.mjs"; // [0808 机制] 前台/后台刷新分级单源（整页最小化时 5s 兜底轮询真暂停）
...
function _setupSmartPolling(active) {
  // [0808 windowRuntime 机制] 业务启停封装保留（本函数仍是唯一开关），interval 换可见性工厂：
  //   整页后台时 5s 兜底轮询真暂停（原 setInterval 最小化仍常醒），转前台立即补拉一次
  //   （非强制 tick，WS 健康 60s 去重照常生效，不产生多余 HTTP）。
  if (active) {
    _triggerIdeApprovalPoll(true); // 业务开启即强制同步（原语义保留）
    if (!_smartPollTimer) {
      _smartPollTimer = createVisibilityPoller({ tick: _triggerIdeApprovalPoll, intervalMs: 5000, label: "smartApprovalPoll" });
      _smartPollTimer.start();
    }
  } else if (_smartPollTimer) {
    _smartPollTimer.stop();
