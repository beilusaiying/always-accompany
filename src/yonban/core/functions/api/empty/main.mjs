...
import info_dynamic from "../../../../../public/parts/serviceGenerators/AI/empty/info.dynamic.json" with { type: "json" };
import info from "../../../../../public/parts/serviceGenerators/AI/empty/info.json" with { type: "json" };
...
async function GetSource(config) {
  console.warn(
    `[AI源诊断] ⚠️ empty GetSource 被调用！调用栈:`,
    new Error("empty GetSource called").stack,
  );
  const error = new Error(
    "This is an empty AI source, which is a placeholder for a previously used generator that error in loading or has been uninstalled or renamed. Please select a new generator.",
  );
  /** @type {AIsource_t} */
  const result = {
    type: "text-chat",
