...
export async function buildPromptStruct(args, detail_level = 3) {
  ...
  if (world?.interfaces?.chat)
    result.world_prompt = world.interfaces.chat.GetPrompt(args);
  if (user?.interfaces?.chat)
    result.user_prompt = user.interfaces.chat.GetPrompt(args);
  if (char?.interfaces?.chat)
    result.char_prompt = char.interfaces.chat.GetPrompt(args);
  for (const other_char of Object.keys(other_chars))
    result.other_chars_prompt[other_char] =
      other_chars[other_char].interfaces.chat?.GetPromptForOther?.(args);
  for (const plugin of Object.keys(plugins))
    result.plugin_prompts[plugin] =
      plugins[plugin].interfaces.chat?.GetPrompt?.(args);
  ...
  while (detail_level--)
    await Promise.all([
      world?.interfaces?.chat?.TweakPrompt?.(
        args,
        result,
        result.world_prompt,
        detail_level,
      ),
      user?.interfaces?.chat?.TweakPrompt?.(
        args,
        result,
        result.user_prompt,
        detail_level,
      ...
      ...Object.keys(other_chars).map((other_char) =>
        other_chars[other_char].interfaces.chat?.TweakPromptForOther?.(
          args,
          result,
          result.other_chars_prompt[other_char],
          detail_level,
      ...
      ...Object.keys(plugins).map((plugin) =>
        plugins[plugin].interfaces.chat?.TweakPrompt?.(
          args,
          result,
          result.plugin_prompts[plugin],
          detail_level,
