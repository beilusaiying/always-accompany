...
平移保真（参数全取 JS 现值，env 名同款）：
    ...
    ANCHOR_MAX=8 / ANCHOR_BCC_DEMOTE=300000 / combinedMin 默认4(configured 优先) / contextCount 默认5
...
def recall_node0(chat_history, input_swow, input_centroid, nb_pool, is_noise, seen_nodes,
    ...
    context_count = max(0, min(20, int(configured_context_count))) \
        if configured_context_count is not None else 5

    # ── Step 4: 取最近 N 句用户上下文 ──
    ctx_sentences = []
    if isinstance(chat_history, list):
        user_history = [m for m in chat_history if isinstance(m, dict) and m.get("role") == "user"]
        # JS slice(-contextCount): contextCount=0 时 slice(-0)=全量 —— python [-0:] 同语义
        for m in user_history[-context_count:]:
