...
export function normalizeCommandWatchdogPolicy(raw = {}) {
  const source = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
  const action = source.command_watchdog_action === "report" ? "report" : "terminate";
  const sampleMs = Math.round(_boundedNumber(source.command_watchdog_sample_ms, 5000, 500, 60000));
  return {
    ...
    stallMs: Math.max(
      sampleMs * 2,
      Math.round(_boundedNumber(source.command_watchdog_stall_ms, 60000, 3000, 3600000)),
    ),
    cpuDeltaThresholdMs: _boundedNumber(
      source.command_watchdog_cpu_delta_ms,
      10,
      0,
      60000,
    ),
    rssDeltaThresholdBytes: Math.round(_boundedNumber(
      source.command_watchdog_rss_delta_bytes,
      1048576,
      0,
      1073741824,
    )),
    progressMs: Math.round(_boundedNumber(
...
function _execFile(file, args, timeout = 4000) {
...
async function _sampleWindows(rootPid) {
  const script = [
  ...
  const stdout = await _execFile("powershell.exe", [
    "-NoProfile",
    "-NonInteractive",
    "-ExecutionPolicy",
    "Bypass",
  ...
  return _parseJsonOutput(stdout);
}

function _parsePsCpuTime(value) {
...
async function _samplePosix(rootPid) {
  const stdout = await _execFile("ps", ["-axo", "pid=,ppid=,time=,rss="]);
  const rows = [];
...
export async function sampleProcessTree(rootPid) {
  ...
  try {
    const sample = process.platform === "win32"
      ? await _sampleWindows(pid)
      : await _samplePosix(pid);
    return {
      available: true,
      rootPid: pid,
    ...
    return {
      ...
      gpuAvailable: false,
      sampledAt: new Date().toISOString(),
      error: String(error?.message || error || "process_telemetry_unavailable").slice(0, 240),
    };
  }
}

function _safeEmit(callback, payload) {
  if (typeof callback !== "function") return;
  try { callback(payload); } catch { /* progress must not break command execution */ }
}

/**
 * 启动连续静默检测。只有可用采样连续证明“无输出/CPU/RSS/进程树变化”才触发 onStall。
 */
export function startCommandWatchdog({
  ...
  const tick = async () => {
    ...
    const active = outputDeltaBytes > 0
      || (previousSample && cpuDeltaMs > policy.cpuDeltaThresholdMs)
      || (previousSample && rssDeltaBytes > policy.rssDeltaThresholdBytes)
      || processCountChanged;
    if (active) {
    ...
    if (sampledAtMs - lastProgressAt >= policy.progressMs || firstStall) {
      lastProgressAt = sampledAtMs;
      _safeEmit(onProgress, progress);
    }
    if (progress.phase === "stalled" && !stalledReported) {
      stalledReported = true;
      _safeEmit(onStall, { ...progress, errorCode: "command_stalled" });
