...
export async function renderTemplateNoScriptActivation(template, data = {}) {
  ...
  if (!template_cache[template]) _tDiag("log", "fetch:", templatePath + "/" + template + ".html");
  // [0723 缓存投毒修] 失败的 fetch Promise 必须从缓存驱逐：原 `??=` 把 rejected promise 永久留在
  //   template_cache → 一次瞬时失败(如 401 会话失效)后该模板永远重抛缓存旧错,会话恢复也修不好,
  //   直到刷新页面(0723 编辑消息大片空白案实证:message_edit_view 401 后编辑功能整体失效)。
  //   驱逐后下次调用重新 fetch=自愈;成功路径不变(并发仍共享同一 in-flight promise)。
  template_cache[template] ??= fetch(
    templatePath + "/" + template + ".html",
  ).then((response) => {
    if (!response.ok) {
      _tDiag("error", "fetch failed:", template, "status:", response.status);
    ...
    find: while (html.indexOf("}", end_index) != -1) {
      ...
      try {
        ...
        if (!(error instanceof SyntaxError)) {
          _templateFailCount++;
          _tDiag("warn", "expr eval failed:", template, "expr:", expression.slice(0, 80), "err:", error?.message);
        }
      }
    }
    if (!_exprResolved) {
      _templateFailCount++;
      _tDiag("warn", "expr unresolved (no closing } matched):", template, "near:", html.slice(0, 60));
    }
  }
  _tDiag("log", "render:", template, "exprs resolved:", _templateExprCount, "failed:", _templateFailCount);
