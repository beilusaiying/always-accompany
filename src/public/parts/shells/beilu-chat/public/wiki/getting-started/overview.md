...
| 从聊天切到编程或工作 | 角色与公共能力可以延续，Code/Work 的任务数据又按模式隔离 |
...
贝露的优势不是每一个单项都胜过专用产品，而是这些能力围绕同一个长期 AI 协同。

## 选择一条入门路径

### 路线 A：先聊天，再决定要不要深入

1. 按[安装指南](install.md)启动；
2. 配置一个 AI 服务源；
3. 导入或创建角色卡；
4. 在 Chat 模式完成[第一次对话](first-chat.md)；
5. 打开回复下方的记忆溯源卡，观察本轮实际使用了哪些记忆。
...
适合：想自定义角色行为、游戏/叙事系统、传感器或本地服务的人。
...
<div class="wiki-arrow">↓</div>
<div class="wiki-box wiki-box-amber wiki-box-full"><b>本模式存储 / 执行端</b><small>角色记忆可以延续，任务与异步作业保持模式和 chatId 归属</small></div>
<div class="wiki-arrow">↓</div>
...
P2 的后台自动触发已经停止。当前手动按钮路径仍存在已知问题，会在 manual_button 判断处提前返回。因此现阶段不要把 P2 当成“默认自动总结”或稳定卖点。

机械归档与 P2 的 AI 总结是两条不同链路；前者不等于后者已经工作。
