...
export function initVariableManager(container) {
	...
	_msgHandler = (e) => {
		...
		// N15：origin 校验（镜像 iframeRenderer.mjs:278）。srcdoc iframe origin="null"、同源=location.origin；拒其它
...
function _renderNormalToolbar(wrap) {
	...
	;['tree', 'text'].forEach(mode => {
		...
		btn.textContent = mode === 'tree' ? '树状' : '文本'
	...
	searchInput.className = 'var-search-input'
	searchInput.type = 'text'
	searchInput.placeholder = '搜索变量...'
	searchInput.value = _searchKeyword
	...
	if (_searchKeyword) {
		...
		clearBtn.className = 'var-search-clear'
...
function _renderNormalView(content) {
	let data = _getActiveData()
	content.innerHTML = ''

	if (_searchKeyword) {
		data = _filterData(data, _searchKeyword)
	}

	if (_viewMode === 'tree') {
		if (!data || (typeof data === 'object' && Object.keys(data).length === 0)) {
			content.innerHTML = _searchKeyword
				? '<div class="var-empty">未找到匹配变量</div>'
				: '<div class="var-empty">（空）</div>'
		} else {
...
function _renderFloorPanel(floorId, data) {
	...
	if (isExpanded) {
		...
		if (_searchKeyword) {
			displayData = _filterData(data, _searchKeyword)
		}
		const filteredCount = typeof displayData === 'object' ? Object.keys(displayData).length : 0

		if (viewMode === 'tree') {
			if (filteredCount === 0) {
				contentArea.innerHTML = _searchKeyword
					? '<div class="var-empty">未找到匹配变量</div>'
					: '<div class="var-empty">（空）</div>'
			} else {
				const tree = _renderFloorTree(floorId, displayData, '', 0)
...
function _startInlineEdit(span, originalValue, path) {
	...
	// _done：提交/取消一次性闸。commit 会触发 _renderContent 移除 input → blur 再次 commit；
	//   Esc 取消同样靠重渲染移除 input → blur commit 把"取消"覆盖回写。置位后两类二次 blur 均 no-op。
	let _done = false
	const commit = () => {
		if (_done) return
		_done = true
...
function _setValueByPath(path, value) {
	...
	// 楼层(message)作用域：楼层树叶子路径被前缀成 `floor:${floorId}:${realPath}`(见 _renderFloorTree:702/764)。
	//   正解 = 写回该楼对象的 realPath，用 scope='message' + key=floorId 落到 _vars.messages[floorId]
	//   (variableStore.replaceVariables case 'message')。旧实现 split('.') 不识别 `:` 前缀→把整串当顶层 key，
	//   且 _getActiveData() 在 messages tab 返回的是整个 messages 映射、scope 又落 'chat' → 脏 key 污染 chat 作用域(BUG-3)。
	const floorMatch = /^floor:([^:]+):(.+)$/.exec(path)
	if (floorMatch) {
		const floorId = floorMatch[1]
		const floorData = { ...(_getMessagesData()[floorId] || {}) }
		_assignByDotPath(floorData, floorMatch[2], value)
		replaceVariables(floorData, { scope: 'message', key: floorId })
		return
	}

	const scopeData = _getActiveData()
	_assignByDotPath(scopeData, path, value)
	replaceVariables(scopeData, { scope: _activeTab })
}

// 按点分路径写入嵌套值（不存在的中间层补 {}）。
function _assignByDotPath(obj, path, value) {
...
function _filterData(data, keyword) {
	if (!data || typeof data !== 'object' || !keyword) return data
	const lowerKw = keyword.toLowerCase()
	const result = {}
	for (const [key, val] of Object.entries(data)) {
		const keyMatch = key.toLowerCase().includes(lowerKw)
		const valMatch = typeof val === 'string' && val.toLowerCase().includes(lowerKw)
		if (keyMatch || valMatch) {
			result[key] = val
		} else if (val && typeof val === 'object') {
			const sub = _filterData(val, keyword)
...
.var-tab:hover { color: oklch(var(--bc) / 0.85); background: oklch(var(--bc) / 0.04); }
.var-tab-active {
	color: var(--beilu-amber);
	border-bottom-color: var(--beilu-amber);
	background: var(--beilu-amber-6);
}
...
.var-search-input:focus { border-color: var(--beilu-amber); }
.var-search-input::placeholder { color: oklch(var(--bc) / 0.65); }
.var-search-clear {
	width: 20px;
	height: 20px;
	display: flex;
...
.var-search-clear:hover { color: #f14c4c; background: rgba(241,76,76,0.1); }
...
.var-view-btn:hover { color: oklch(var(--bc) / 0.85); }
.var-view-btn-active {
	color: var(--beilu-amber);
...
.var-tool-btn:hover { color: var(--beilu-amber); border-color: var(--beilu-amber-30); background: var(--beilu-amber-6); }
.var-tool-btn-sm {
	width: 22px;
	height: 22px;
	font-size: 11px;
...
.var-toggle:hover { color: var(--beilu-amber); }
...
.var-val-null { color: oklch(var(--bc) / 0.65); font-style: italic; }
.var-val-empty { color: oklch(var(--bc) / 0.65); }
.var-bool-indicator { margin-right: 2px; font-size: 11px; }

/* ── 内联编辑 ── */
.var-inline-edit {
	background: oklch(var(--b1) / 0.8);
	border: 1px solid var(--beilu-amber);
	border-radius: 3px;
	color: oklch(var(--bc));
	font-family: inherit;
	font-size: 11px;
...
.var-floor-track-btn:hover { color: oklch(var(--bc) / 0.85); border-color: oklch(var(--bc) / 0.2); }
...
.var-floor-range-input:focus { border-color: var(--beilu-amber); }
.var-floor-range-sep {
	color: oklch(var(--bc) / 0.65);
	font-size: 11px;
}
...
.var-floor-expanded:hover {
	border-color: var(--beilu-amber-30);
}
...
.var-floor-expanded .var-floor-chevron { color: var(--beilu-amber); }
...
.var-floor-expanded .var-floor-title { color: var(--beilu-amber); }
...
/* 楼层内容 */
.var-floor-body {
	border-top: 1px solid oklch(var(--bc) / 0.04);
	background: oklch(var(--b1) / 0.5);
}
...
.var-floor-inner-toolbar {
	...
	align-items: center;
	padding: 3px 8px;
	border-bottom: 1px solid oklch(var(--bc) / 0.03);
