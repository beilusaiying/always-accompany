...
为什么不做成关键词检索？因为人的记忆回想不是检索——给一个关键词马上想起相关记忆，靠的是**发散 + 检索**：简单检索做不到广度，而记忆越多、全量翻找的开销越大。这也是正在打磨的下一代方案（[P1 自驱动召回](../p1-recall/preface.md)：SWOW 联想发散 + 六轴定位 + BLQ 打分，零 LLM、约 200ms）要根治的问题；鉴于当前稳定性还在提升，现役方案先由辅助 AI 承担这一步。
...
或许你看过《底特律：变人》，或许看过《可塑性记忆》——里面的人形 AI 工作陪伴一体。要做那样的东西，第一个问题就是记忆：你不想让你的 AI 忘记你们之间的记忆，但按现有方案（暴力压缩 / 塞向量库 / 越堆越多）这几乎不可避免。

那记忆到底是什么？人的记忆其实很短暂——两天前的细节基本就模糊了。但给一个关键词，马上能想起对应的、或相关的记忆。这引出两个方向：**怎么存，怎么找。**

人不会记住每个细节，会**选择性遗忘**；但你也不可能马上忘记最近发生的事，更不会每天把这几年做过的事想一遍。所以：

- **data 表格**存当天的记忆 + 永久的记忆——就像你可能永远记得初恋的名字、记得第一件事、记得告白
- **热层（按周）**：每天的 data 自动归档，按时间、事件、流程的线归档
- **温层（按月）**：二次压缩、提取关键词，像一本目录——热层里留着温层内容的大概描述
- **冷层（按年）**：目录式过渡，深度归档但检索可达
- chat / code / work 各有自己的 data，工作和陪伴互不干扰

**注入权重按层级递减**：上下文 > data（永久记忆、轮回条目）> 热层 > 温层 > 冷层；同时按最近召回情况在每个层级内做 top-k 二次排序，层级之间还有缓冲层。一个完整的模拟记忆召回层 + 一个动态层。

按 AI 录入 data 的实际情况和每天的归档优化推导，运行一年后每轮注入依然小于 1 万 token（推导值：一条 data 条目 ≈20 字符、每天 ~100 次互动、每日 AI 总结优化）。而且除了少数难点，这套东西**纯提示词 + 纯 JSON 文档驱动**——改归档策略、表格含义、检索风格，改提示词就行。存储成本≈0。
