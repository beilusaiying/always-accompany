/**
 * edit-tools.mjs -- 局部编辑工具（replace_lines / insert_at_line / fuzzy_edit）
 *
 * ═══════════════════════════════════════════════════════════════
 ...
 *   → infra.mjs resolveWorkspacePath/assertWritable/autoSyntaxCheck/verifyWrite/detectCallSites
 *   → FileCheckpoint（写前快照）
 *   → constants.mjs MAX_EDIT_FILE_SIZE
 */
...
function _normalizeInnerWs(s) {
  return s.replace(/\s+/g, " ").trim();
}

/** LineTrimmed：返回所有匹配的起始行下标（去前后空白后逐行比对），供唯一性校验 */
function _findAllTrimmedMatches(contentLines, oldLines) {
  const out = [];
...
function _findAllWsNormMatches(contentLines, oldLines) {
  const out = [];
  if (oldLines.length === 0) return out;
...
function _findAllBlockAnchorMatches(contentLines, oldLines) {
  const out = [];
 ...
 */
function _pickUnique(starts, lineHint) {
...
/** 查找最接近的匹配位置（用于错误提示） */
function _findClosestMatches(contentLines, oldLines, maxSuggestions) {
 ...
 */
async function _anchorInsert(params, checkpoint) {
...
/** 替换文件指定行范围的内容 */
export async function replaceLines(params, checkpoint) {
  const filePath = params.path;
...
/** 在文件指定行位置插入内容 */
export async function insertAtLine(params, checkpoint) {
  const filePath = params.path;
  if (!filePath) throw new Error("缺少 path 参数");
 ...
 */
export async function fuzzyEdit(params, checkpoint) {
  const filePath = params.path;
