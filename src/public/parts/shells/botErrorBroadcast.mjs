 ...
 *       （broadcast.mjs / broadcastAllChatUi）以结构化 `bot_error` 事件推给所有
 *       已连接的聊天 UI，前端 botSidePanels [O] 监控红点据此计数。
 *
 * 设计依据：
 *   - 复用既有通道：与 replyHandler `_broadcastCloneStatus` 同型（动态 import
 *     broadcast.mjs，避免在 bot 壳内静态依赖 chat 壳造成加载耦合）。
 *   - 选 broadcastAllChatUi（非 broadcastChatEvent）：Bot 进程无 chatId 概念，
 *     错误是全局事件，任何打开的 UI 都应看到红点 → 走「全 UI 广播」而非按 chatId 定向。
...
// shells/botErrorBroadcast.mjs → src/server/web_server/event_dispatcher.mjs（通道B 桌面通知）
 ...
 */
export function broadcastBotError({ platform, botname, phase = "runtime", error }) {
  try {
    const message =
    ...
    // 桌面 OS 通知（通道B sendEventToAll → 各页 base.mjs onServerEvent('notification') → notifyDesktop）。
    ...
    if (phase === "start" || phase === "connect") {
      import(pathToFileURL(_eventDispatcherPath).href)
        .then((_ed) => {
          if (typeof _ed.sendEventToAll !== "function") return;
          _ed.sendEventToAll("notification", {
