...
// [0807 插件化搬迁] 实现体迁回 plugins/beilu-worldbook/main.mjs（T3d 曾迁入 core，凛倾 0806"改成框架,
//   而不是耦合核心"定向后与 mvu/ejs 同款回迁）。core 侧 functions/prompt/index.mjs 只留惰性 facade——
//   删本 part 目录 = worldbook 功能诚实缺席（wb 系 verbs 报不可用），主链照常（"删了还能跑"）。
...
// ============================================================
// [0807 世界书缺口#1] ST 内容装饰器 @@activate / @@dont_activate
// 对齐 ST 本体 world-info.js parseDecorators(:4540-4586) 逐字语义 + KNOWN_DECORATORS(:100)：
//   · content 以 @@ 开头才进入解析；起始连续 @@ 行是装饰器区，首个非 @@ 行起为正文
//   · @@@ 行=转义/fallback：前一行不是未知装饰器(fallbacked=false)时跳过；
//     fallbacked=true 时作为候选，命中已知集则去掉一个 @ 入列
//   · 未知 @@ 行置 fallbacked=true（自身被剥离出正文，不入装饰器列）
//   · 边界行为(ST 原样保留)：全部行都是 @@ 行(无正文行)时 newContent 保持原文不剥
// 级联位置对齐 checkWorldInfo(:4763-4772)：装饰器判定先于 constant/sticky/关键词——
//   @@dont_activate 压制一切(含 constant/sticky 条目)；@@activate 直接激活但概率检验仍在其后
//   (verifyProbability:4909，sticky 免掷不适用于此路径——被压制条目不进引擎，无 sticky 状态可言)。
// 消费点：GetPrompt / GetRenderEntries 两处，均在阶段过滤后、三模式分流前应用。
// ============================================================
const WI_KNOWN_DECORATORS = ["@@activate", "@@dont_activate"];

/**
 * @param {string} content
 * @returns {[string[], string]} [装饰器列表, 剥离装饰器区后的正文]
 */
function parseEntryDecorators(content) {
  const isKnownDecorator = (data) => {
    if (data.startsWith("@@@")) data = data.substring(1);
    return WI_KNOWN_DECORATORS.some((d) => data.startsWith(d));
  };
  if (content.startsWith("@@")) {
    ...
    for (let i = 0; i < splited.length; i++) {
      if (splited[i].startsWith("@@")) {
        if (splited[i].startsWith("@@@") && !fallbacked) continue;
...
/**
 * 对条目数组应用装饰器语义：@@dont_activate 剔除；@@activate 标记 _decoratorActivate(过概率)
 * 并剥离装饰器区。不改 store 内对象（命中才浅拷贝副本）。
 * @param {Array<object>} entries
 * @returns {Array<object>}
 */
function applyEntryDecorators(entries) {
  const out = [];
  for (const e of entries) {
    const c = e.content || "";
    if (!c.startsWith("@@")) { out.push(e); continue; }
    const [decorators, newContent] = parseEntryDecorators(c);
    if (decorators.includes("@@activate")) {
      // ST 级联：@@activate 直接激活，但 verifyProbability 仍在其后——此处同 roll
      if (e.useProbability && (e.probability ?? 100) < 100 && Math.random() * 100 > (e.probability ?? 100)) continue;
      out.push({ ...e, content: newContent, _decoratorActivate: true });
      continue;
    }
    if (decorators.includes("@@dont_activate")) continue;
    out.push(newContent !== c ? { ...e, content: newContent } : e);
  }
  return out;
}
...
import { resolveGenerationMode, getModeCtxDir, getTablesFileName, getUserDataDir } from "../../../../yonban/core/functions/memory/storage_mod/storage.mjs"; // T8·回切：改组内引用（T3a 暂指旧位壳的欠账，T3e memory 已入住）；resolveGenerationMode=0715 生成链 mode 单源收口（原 getActiveMode 不看 platform）
...
const __pluginDir = dirname(fileURLToPath(import.meta.url)); // [0807 插件化] 实现体已在 part 目录，自锚（值与迁移前恒等）
...
// [0806 无损透传] ST→beilu 转换是**白名单式**的：下方 convertSTEntry 逐个具名字段读取，
//   任何不在清单里的字段（ST 新版新增字段 / 第三方插件写进条目的自定义数据）都会被
//   createBlankEntry 默认值静默覆盖、不落盘、导出时也回不去。本文件历史上已因此犯过一次
//   （见 convertSTEntry 内 "ST 高级字段无损保留(此前漏映射…)" 注释——当时的修法是再补几行
//   具名映射，没根治白名单本身，所以下一个新字段照丢）。
//   根治：把**未被具名消费**的字段原样收进 `_stExtra` 运输容器，导出时铺回原位。
//   _stExtra 不参与任何业务逻辑（无人读取它做判断），只为往返无损；与具名字段互斥，不构成双源。
/** convertSTEntry 已在**条目顶层**具名消费的 ST 键（含同义别名） */
const _ST_CONSUMED_TOP = new Set([
  "uid", "id", "comment", "content", "key", "keys", "keysecondary", "secondary_keys",
  "constant", "useRegex", "use_regex", "selective", "order", "insertion_order",
  "disable", "enabled", "position", "depth", "role", "selectiveLogic",
  ...
  "sticky", "cooldown", "delay", "vectorized", "triggers", "ignoreBudget", "outletName",
  ...
  "activationMode", "dynamicConfig", "extensions",
]);
/** convertSTEntry 已在 **entry.extensions** 内具名消费的 ST 键 */
const _ST_CONSUMED_EXT = new Set([
  "position", "depth", "role", "selectiveLogic", "exclude_recursion", "prevent_recursion",
  "delay_until_recursion", "display_index", "probability", "useProbability", "group",
  "group_override", "group_weight", "scan_depth", "case_sensitive", "match_whole_words",
  "use_group_scoring", "automation_id", "sticky", "cooldown", "delay", "vectorized",
  "triggers", "ignore_budget", "outlet_name", "match_persona_description",
  "match_character_description", "match_character_personality",
  "match_character_depth_prompt", "match_scenario", "match_creator_notes",
]);

/**
 * 收集 ST 条目里未被具名消费的字段（顶层 + extensions 各一份）。
 * @returns {{top?:object, ext?:object}|undefined} 全部字段都已具名消费时返回 undefined（不给条目加空容器）
 */
function _collectSTExtra(raw, ext) {
  const top = {};
  for (const k of Object.keys(raw || {})) if (!_ST_CONSUMED_TOP.has(k)) top[k] = raw[k];
  const e = {};
  for (const k of Object.keys(ext || {})) if (!_ST_CONSUMED_EXT.has(k)) e[k] = ext[k];
  const out = {};
  if (Object.keys(top).length) out.top = top;
  if (Object.keys(e).length) out.ext = e;
 ...
 * 未知字段经 _collectSTExtra 收进 _stExtra 无损保留（见上方说明）
...
function convertSTEntry(raw, fallbackUid = 0) {
  ...
  const _stExtra = _collectSTExtra(raw, ext); // [0806] 未知字段无损容器；全已知时为 undefined
  return {
...
function convertSTEntries(rawEntries) {
  ...
  for (let i = 0; i < rawList.length; i++) {
    ...
    if (entry._stExtra) {
      _extraEntries++;
      for (const k of Object.keys(entry._stExtra.top || {})) _extraKeys.add(k);
      for (const k of Object.keys(entry._stExtra.ext || {})) _extraKeys.add("ext." + k);
    }
    converted[String(entry.uid)] = entry;
  }
  // [0806 可观测] 导入时遇到多少条目带我们不认识的字段、都是哪些键——原来这些字段被静默丢弃，
  //   用户与开发者都无从察觉。现在既无损保留，也在白盒里留痕（前端查看器 line=worldbook 可筛）。
  try {
    wbT(null, "worldbook", "import:stExtra", {
      entries: rawList.length,
      withExtra: _extraEntries,
      keys: [..._extraKeys].slice(0, 30),
    });
  } catch { /* 白盒不可用不阻断导入 */ }
  return converted;
}
...
function convertToSTEntry(e) {
  // [0806 无损透传] 先铺开导入时收下的未知字段，再让具名字段覆盖——
  //   顺序保证具名字段永远权威（_stExtra 只承载"我们不认识的"，与具名键互斥，不构成双源）。
  const _x = e._stExtra || {};
  return {
    ...(_x.top || {}),
    id: e.uid,
    ...
    extensions: {
      ...(_x.ext || {}), // [0806] 同上：未知 extensions 字段先铺，具名字段随后覆盖
  ...
  interfaces: {
    ...
    chat: {
      ...
      GetPrompt: (arg) => {
        ...
        // [0807 世界书缺口#1] 装饰器先于三模式判定（ST checkWorldInfo 级联同位）
        const _preDecorLen = entryArray.length;
        entryArray = applyEntryDecorators(entryArray);
        {
          // 白盒：装饰器压制/强激活计数（前端查看器 line=worldbook 可筛，验收入口）
          const _decorForced = entryArray.filter((e) => e._decoratorActivate).length;
          const _decorSuppressed = _preDecorLen - entryArray.length;
          if (_decorForced || _decorSuppressed) {
            wbT(_cid, "worldbook", "decorators", { forced: _decorForced, suppressed: _decorSuppressed });
          }
        }
        if (entryArray.length === 0) {
          return { text: [], additional_chat_log: [], extension: {} };
        ...
        for (const entry of entryArray) {
          if (entry._decoratorActivate) {
            constantEntries.push(entry); // @@activate 直接激活（概率已在 applyEntryDecorators 检验）
            continue;
          }
          const mode =
            entry.activationMode || (entry.constant ? "constant" : "regex");
      ...
      GetRenderEntries: (arg) => {
        ...
        // [0807 世界书缺口#1] 装饰器（与 GetPrompt 同口径）
        entryArray = applyEntryDecorators(entryArray);
        if (entryArray.length === 0) return { entries: [] };
        ...
        const dynamicEntries = [];
        for (const entry of entryArray) {
          if (entry._decoratorActivate) { constantEntries.push(entry); continue; }
          const mode =
            entry.activationMode || (entry.constant ? "constant" : "regex");
