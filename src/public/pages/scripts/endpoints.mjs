...
// T12：export generateVerificationCode() 已纯删——全库 grep 确认零 import/零调用方（无注册验证码 UI 消费）。
// 其 fetch 的 /api/register/generateverificationcode 路由处置（属注册流程变更）列主 AI 待决，未随本删除动。
 ...
 * @param {string[]} [scopes] - 作用域列表（默认 ['*'] 全权限）。
 * @returns {Promise<object>} - 新的 API 密钥。
 */
export async function createApiKey(description, scopes) {
	const response = await fetch('/api/apikey/create', {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		body: JSON.stringify({ description, ...(Array.isArray(scopes) && scopes.length > 0 ? { scopes } : {}) }),
	})
	if (!response.ok) return Promise.reject(Object.assign(new Error(`API request failed with status ${response.status}`), await response.json().catch(() => ({})), { response }))
	return response.json()
}
...
/**
 * 获取可用的 API Key scope 列表。
 * @returns {Promise<object>} - 可用 scope 列表。
 */
export async function getAvailableScopes() {
	const response = await fetch('/api/apikey/available-scopes')
	if (!response.ok) return Promise.reject(Object.assign(new Error(`API request failed with status ${response.status}`), await response.json().catch(() => ({})), { response }))
	return response.json()
}
