...
> [!NOTE]
> **개발 안내:** 이 프로젝트의 주요 부분은 한 사람이 약 3개월 동안 완성했으며, 이후 약 한 달간 알고리즘 최적화에 집중했습니다. 개발 기간이 짧고 기능 범위가 넓어 현재의 프로젝트 구조, 기본 기능, 경계 조건 처리가 아직 불안정하거나 완성되지 않은 부분이 있을 수 있습니다. 일부 기본 기능은 AI의 도움으로 구현했지만, 복잡한 기능의 프레임워크, 알고리즘, 핵심 설계는 저자가 직접 계획하고 주도했기 때문에 모듈마다 완성도가 다릅니다. 앞으로도 수동 검토, 미세 조정, 엔지니어링 개선을 계속할 예정입니다. 버그를 발견하면 재현 절차와 로그를 제공해 주세요.
>
> **향후 계획:** 새로운 플러그인과 기능 영역의 추가를 중단하고, 코어 축소와 결합도 완화, 분리 가능한 기능의 플러그인 계층 이전에 집중합니다. 상세하고 안정적인 플러그인 프로토콜을 완성한 뒤 프레임워크 수준의 엔지니어링 최적화와 점진적 리팩터링을 진행하며, 테스트·문서·기여 절차도 개선해 더 많은 개발자가 프로젝트를 이해하고 확장하며 참여할 수 있도록 하겠습니다.
...
- **계층형 기억·컨텍스트** — Data + hot / warm / cold 계층으로 장기 자료를 보관하고, 컨텍스트를 수집하고 기억을 검색하는 도구(P1)가 매 턴 응답 전에 현재 관련된 조각을 회상합니다. 컨텍스트 정리는 파일 읽기 단위로 되돌릴 수 있으며, AI 스스로도 더 이상 필요 없는 이미 읽은 파일을 놓을 수 있습니다.
- **핵심 동작을 확인하고 설정 가능** — 캐릭터, 프롬프트, 주입, 기억, 회상 경로, 권한과 플러그인에는 각각 편집 또는 설정 진입점이 있습니다.
- **플러그인 기반 확장 프레임워크** — 핵심 기능은 플러그인으로 조직되어 중간 정보 스테이션을 거쳐 전달되고, 프런트엔드는 표시와 조작을 담당합니다. 사용자 플러그인은 JS, Python 또는 독립 프로그램으로 작성할 수 있습니다.
- **통합된 agent 도구 체계** — 같은 기억·컨텍스트 프레임워크에서 파일, 명령, 브라우저 연동, MCP, 다중 창, 승인과 복원을 제공합니다. 실제 사용 범위와 결과는 선택한 모드, 설정, 실행 환경, 모델과 외부 서비스에 따라 달라집니다.
...
서비스 주소, API Key, 모델을 입력하고 저장한 뒤 캐릭터 카드를 하나 선택하거나 임포트하면 채팅을 시작할 수 있습니다. 최소한 사용 가능한 AI API 하나가 필요하며, 모델 성능과 비용은 바인딩한 서비스에 따라 달라집니다. 앱에는 [Wiki](site/wiki/getting-started/overview.md)가 내장되어 있으며, [온라인 버전](https://beilusaiying.github.io/always-accompany/)도 볼 수 있습니다.
...
- **👑 주요 프롬프트 항목 편집 가능**: 캐릭터 정의, 프리셋, INJ 항목, 모드 설명, 기억 데이터 슬롯과 도구 설명은 해당 편집기가 제공하는 내용, 순서, 켜기/끄기, 역할, 주입 위치 또는 조건을 조정할 수 있습니다. 프레임워크 안전 래퍼와 공급자별 메시지 변환은 코드에서 관리됩니다.
...
이들을 P1(컨텍스트를 수집하고 기억을 검색하는 도구)과 조합하면 "벡터 + 동적 주입 + 기억이 현재 작업을 따라 움직임"이라는 설정 가능한 흐름이 되고, 파일 읽기 단위의 컨텍스트 정리도 같은 경로에 포함됩니다. 실제 회상과 압축 결과는 선택한 모드, 설정, 자료와 모델에 따라 달라집니다.
...
<details>
<summary><strong>👑 주요 프롬프트 항목 편집 가능 — 기본으로 쓸 수 있고, 작업 흐름에 맞게 설정 가능</strong></summary>

캐릭터 정의, 프리셋, INJ 항목, 모드 설명, 기억 데이터 슬롯, 도구 설명 등의 주요 프롬프트 항목은 화면에서 편집할 수 있습니다. 각 항목은 편집기가 실제로 제공하는 설정을 조정할 수 있으며, 프레임워크 안전 래퍼와 공급자별 메시지 변환은 코드에서 관리됩니다.
...
**현재 저장소에 이미 갖춰진 진입점과 구현**: Data + 3계층 기억 · 컨텍스트 관리 · 자율주행 P1 / AI P1 · 프롬프트 항목 편집과 프리셋 전환 · 모드 기억 테이블 · 조건 지식 동적 주입 · Live2D / 이미지 데스크톱 펫 · 화면 인식과 게임 동반 · 로컬 음성 입력 · PPT 생성 · MCP · 다중 창 · VS Code 확장 브리지 · 9개 플랫폼 Bot · 23개 내장 플러그인 디렉터리 · 사용자 플러그인 호스트 · 회수 / 백업 체인 · 화이트박스 진단 · 다국어와 테마.
...
always-accompany는 인기 기능을 한 메뉴에 욱여넣기 위한 것이 아닙니다 — 처음에는 그저 작성자 자신이 쓰고 싶었을 뿐입니다 :). 플러그인·프레임워크 체계와 여러 표시 언어를 제공하지만, 실제 지원 범위는 플러그인과 번역 리소스에 따라 달라집니다.
