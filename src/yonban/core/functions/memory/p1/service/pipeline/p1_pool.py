...
def _get_cfn_data() -> Optional[dict[str, Any]]:
    ...
    try:
        _cfn_fwd = json.loads((res.get_res_dir() / "CFN-Lex" / "CFN_LEX_CN.json").read_text(encoding="utf-8-sig"))
        _cfn_rev = {}
...
# ── 用户插拔词库（按 vocabRoot 分桶 + 30s TTL；enabled 单源在文件内）──
# runP1 通过 asyncio.to_thread 并发执行；单个 cache/key 会让 A/B 用户交错后出现 key=B、内容=A。
# 因此缓存身份必须就是权威 vocabRoot，读/填充/失效共用同一锁，禁止跨用户单槽。
_user_vocab_cache_by_root: dict[str, tuple[float, list[dict[str, Any]]]] = {}
_user_vocab_cache_lock = threading.RLock()


def _get_user_vocab(vocab_root: Optional[str] = None) -> list[dict[str, Any]]:
    _vocab_dir = Path(vocab_root) if vocab_root else res.get_user_vocab_dir()
    cache_key = str(_vocab_dir.resolve())
    with _user_vocab_cache_lock:
        now = time.time()
        cached = _user_vocab_cache_by_root.get(cache_key)
        ...
        try:
            for f in os.listdir(_vocab_dir):
                ...
                try:
                    ...
                    raise RuntimeError(f"P1 user vocab is unreadable: {_vocab_dir / f}: {error}") from error
        except OSError:
            pass  # vocab 目录不存在=用户未用此功能, 合法路径
        _user_vocab_cache_by_root[cache_key] = (now, out)
        return out
...
def route_mode_res(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if _is_mech_active(mode, "user"):
        vocabs = [v for v in _get_user_vocab(ctx.get("vocabRoot")) if "all" in v["modes"] or mode in v["modes"]]
        if vocabs:
            before = len(words)
...
def get_recall_pool_cache_stats() -> dict[str, Any]:
    ...
    with _user_vocab_cache_lock:
        user_vocab_files = sum(len(entries) for _, entries in _user_vocab_cache_by_root.values())
        user_vocab_roots = len(_user_vocab_cache_by_root)
    return {
        ...
        "userVocabFiles": user_vocab_files,
        "userVocabRoots": user_vocab_roots,
    }


def clear_user_vocab_cache(vocab_root: Optional[str] = None) -> dict[str, Any]:
    """定点失效一个用户的 vocabRoot；无 root 仅供 deep unload 清全部用户词库缓存。"""
    cache_key = str(Path(vocab_root).resolve()) if vocab_root else None
    with _user_vocab_cache_lock:
        if cache_key is not None:
            cached = _user_vocab_cache_by_root.pop(cache_key, None)
            return {
                "scope": "one-root",
                "cacheKey": cache_key,
                "loaded": cached is not None,
                "userVocabFiles": len(cached[1]) if cached else 0,
            }
        before = {
            "scope": "all-roots",
            "userVocabRoots": len(_user_vocab_cache_by_root),
            "userVocabFiles": sum(len(entries) for _, entries in _user_vocab_cache_by_root.values()),
        }
        _user_vocab_cache_by_root.clear()
        return before
...
clearUserVocabCache = clear_user_vocab_cache
