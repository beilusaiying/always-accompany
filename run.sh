...
BEILU_AUTO_INSTALLED_PACKAGES="${BEILU_AUTO_INSTALLED_PACKAGES:-}"
...
install_package() {
	...
	if command -v "$_command_name" >/dev/null 2>&1; then
		# 跟踪自动安装的包
		case ";$BEILU_AUTO_INSTALLED_PACKAGES;" in
		*";$_installed_pkg_name;"*) ;; # 已跟踪
		*)
			if [ -z "$BEILU_AUTO_INSTALLED_PACKAGES" ]; then
				BEILU_AUTO_INSTALLED_PACKAGES="$_installed_pkg_name"
			else
				BEILU_AUTO_INSTALLED_PACKAGES="$BEILU_AUTO_INSTALLED_PACKAGES;$_installed_pkg_name"
			fi
			;;
		esac
		export BEILU_AUTO_INSTALLED_PACKAGES
		return 0
	else
...
# 4. 使用找到的 bash 执行主脚本
# beilu-always-accompany.sh 脚本的路径是相对于 run.sh 脚本的
if [ "$#" -eq 0 ]; then
	"$BASH_EXEC" "$SCRIPT_DIR/path/beilu-always-accompany.sh" open keepalive
else
	"$BASH_EXEC" "$SCRIPT_DIR/path/beilu-always-accompany.sh" "$@"
fi
