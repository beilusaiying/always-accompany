...
import { BUILTIN_MARKERS, SYSTEM_LEVEL_ID, USER_LEVEL_ID, DEFAULT_INJECTION_DEPTH } from './preset_engine.mjs';
...
function cleanPresetData(json, warnings) {
	...
	for (const entry of (json.prompts || [])) {
		...
		cleaned.prompts.push({
			...
			injection_depth: entry.injection_depth ?? DEFAULT_INJECTION_DEPTH,
	...
	if (!cleaned.prompt_order || !Array.isArray(cleaned.prompt_order)) {
		...
		// ST 序组语义归一（0715，凛倾0713「酒馆的预设导入我们这边,位置完全对应不上.尤其是系统条目」根因）：
		//   ST 运行时 global 策略只消费 dummyId=100001 组（openai.js:690 覆盖 PromptManager 类默认 100000），
		//   预设文件里的 100000 组是旧版残留、ST 自己不渲染。透传两组会被我们引擎 buildAllEntries 的
		//   自家双层兜底合并语义吃进去——100001 里被用户禁用、100000 里 enabled 的条目（实测样本 jailbreak）
		//   幽灵复活且追加到序列尾部=系统条目位置错乱。归一规则=复刻 ST 读取语义：
		//   有 100001 组 → 它是唯一真序，丢弃 100000 残留组；只有 100000（旧版 ST 预设）→ 改标 100001 作唯一序。
		//   （自家预设不经本导入器，双层分工语义不受影响。）
		const _stOrderGroups = cleaned.prompt_order.filter(g => g && Array.isArray(g.order));
		const _stUser = _stOrderGroups.find(g => g.character_id === USER_LEVEL_ID);
		const _stSys = _stOrderGroups.find(g => g.character_id === SYSTEM_LEVEL_ID);
		if (_stUser && _stSys) {
			cleaned.prompt_order = _stOrderGroups.filter(g => g.character_id !== SYSTEM_LEVEL_ID);
			warnings.push('已丢弃 ST 旧版残留序组（character_id=100000）——ST 运行时只消费 100001 组');
		} else if (_stSys && !_stUser) {
			_stSys.character_id = USER_LEVEL_ID;
			warnings.push('旧版 ST 预设：100000 序组已归一为 100001 唯一真序');
		}
