...
try:
    ...
    from . import p1_pool as _p1_pool
except ImportError:
    from p1_whitebox import trace as wb_trace, get_trace, clear_trace
    from p1_step1_bracket import split_bracket_channels
    from p1_recall import recall_v2, clear_recall_caches
...
def get_p1_runtime_cache_stats() -> dict:
    ...
    stats["pool"] = _p1_pool.get_recall_pool_cache_stats()
    return stats
...
def clear_p1_runtime_caches(opts=None) -> dict:
    ...
    actions["pool"] = {"status": "cleared", "result": _p1_pool.clear_recall_pool_caches()}
...
def invalidate_user_vocab_cache(vocab_root=None) -> dict:
    """用户词库 CRUD 的定点失效口；不清其他 P1 运行时缓存。"""
    return {"status": "cleared", "result": _p1_pool.clear_user_vocab_cache(vocab_root)}
...
def run_pipeline(input_text, chat_history, mode, user_ctx=None, runtime_config=None) -> dict:
    ...
    if user_ctx and user_ctx.get("username") and user_ctx.get("charName"):
        try:
            memory_root = Path(user_ctx["memoryRoot"]) if user_ctx.get("memoryRoot") else None
            _recall_mem_dirs = ([
                memory_root / "chars" / user_ctx["charName"] / "memory",
                memory_root / "chars" / "_global" / "memory",
            ] if memory_root else [
    ...
    recall_opts = {
        ...
        "vocabRoot": user_ctx.get("vocabRoot") if user_ctx else None,
    }
    try:
