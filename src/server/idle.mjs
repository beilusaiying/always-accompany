/**
 * idle.mjs — 系统空闲状态管理器。不管具体空闲任务内容（那是注册方的事）。
 *
 * 链路：server.mjs init() → idleManager.start()（启动时 L317）
 *        → server.mjs 注册 onIdle 回调（设置终端标题 + 堆内存 EMA 记录）
 *        → 内部 setTimeout 轮询 → isIdle() 判定 → 执行所有 onIdle / onIdleOnce 回调
 * 影响：setTimeout 定时器（.unref() 不阻止进程退出）
 * 相交：← server.mjs(start/stop/markBusy/onIdle)
 *         / index.mjs(.noidle 热开关文件 → stop/start)
 *       → 无下游依赖（纯调度器）
 *
 * 空闲判定：无 runAction 在执行 且 距最后一次 markBusy 超过 timeout（默认 30s）。
 * 检查间隔：默认 5 分钟轮询一次。
 */
 ...
 * 导出全局单例 idleManager（模块尾部），整个进程共享一个实例。
 */
