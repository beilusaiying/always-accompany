...
to the architecture of beilu Shells and the standard procedure for creating new
ones.
...
maintainable, and integrate correctly with the beilu ecosystem.

**See also**: [beilu Architecture & AI Agent Prompting Guide](../../../../AGENTS.md) (root); [Frontend Common Functions Guide](../../pages/AGENTS.md) for shared scripts (theming, i18n, API helpers, etc.).

---

## 1. Standard Shell Architecture & URL-to-Filesystem Mapping

A beilu Shell is a self-contained module that provides a user interface (frontend) and corresponding backend logic. It's crucial to understand how URLs map to the shell's files.

### 1.1. URL Structure: The Colon (`:`) Separator

beilu uses a special colon-based URL structure to serve a shell's public static files (HTML, CSS, JS, images). The format is:
...
1. **`main.mjs` (Backend Entry Point)**
   - **Role**: The Shell's manifest and lifecycle manager. This is how the beilu
...
2. **`public/` (Frontend Directory)**
   ...
   - **`index.html`**: The main page structure. It should include standard beilu
...
4. **`home_registry.json` (Home Shell Integration)**
   - **Role**: Makes your Shell discoverable and accessible from the main beilu
     `home` shell UI.
...
5. **`argument_completer.ps1` (CLI Integration)**
   - **Role**: Provides context-aware command-line completion for users
     interacting with your shell via the beilu CLI (`beilu run shells ...`).
...
```bash
# From beilu root directory, create directories for a shell named "my-new-shell"
...
export function setEndpoints(router) {
  ...
  // The 'ws' method is added by the 'express-ws' middleware used in beilu
  router.ws(
...
When `index.html` is loaded from the URL `/parts/shells:my-new-shell:`, the browser correctly resolves the relative path `./index.mjs` to `/parts/shells:my-new-shell/index.mjs`, which the beilu server then maps to the correct file in the `public` directory.
...
After completing these steps, restart the beilu server. The new shell will be available at the URL `/parts/shells:my-new-shell`, which serves the shell's `index.html`.
