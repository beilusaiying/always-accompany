...
import { assertSafeUrl, safeFetch } from '../../../../yonban/core/functions/security/safe_fetch.mjs'
...
export async function downloadCharacter(url) {
	// SEC-T7：角色卡导入下载 SSRF 防护——拦内网/云元数据(公网角色卡站正常放行)
...
async function downloadChubCharacter(id) {
	...
	const result = await safeFetch(`https://api.chub.ai/api/characters/${creatorName}/${projectName}`, {
		method: 'GET',
	...
	// SEC-T7（D6 二级 URL 漏网）：max_res_url 来自远程响应正文，可被投毒/MITM 指向内网——下载前过闸
...
async function downloadPygmalionCharacter(id) {
	...
	try {
		...
		// SEC-T7（D6 二级 URL 漏网）：avatar 来自远程响应正文——下载前过闸防内网/元数据
		await assertSafeUrl(avatarUrl)
		const avatarResult = await safeFetch(avatarUrl)
...
async function downloadJannyCharacter(uuid) {
	...
	// SEC-T7（D6 二级 URL 漏网）：downloadUrl 来自远程响应正文——下载前过闸防内网/元数据
	await assertSafeUrl(downloadResult.downloadUrl)
	const imageResult = await safeFetch(downloadResult.downloadUrl)
...
async function downloadGenericPng(url) {
	const response = await safeFetch(url, { method: 'GET', headers: { Accept: '*/*' } }) // Set Accept header to allow various content types（safeFetch 含重定向校验）
	if (!response.ok)
...
async function downloadGithubCharacter(id) {
	const result = await safeFetch(`https://api.github.com/repos/${id}/releases/latest`)
	...
	for (const type of contentTypes) {
		...
		if (asset) {
			// SEC-T7（D6 二级 URL 漏网）：browser_download_url 来自远程响应正文——下载前过闸
			await assertSafeUrl(asset.browser_download_url)
			return (await safeFetch(asset.browser_download_url)).arrayBuffer()
		}
	}
