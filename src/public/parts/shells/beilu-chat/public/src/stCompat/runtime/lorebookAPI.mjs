...
export function generateLorebookAPIScript() {
	// R1-SKIP: 下方 return 的是注入 iframe 的脚本字符串(无 ESM,不能 import apiFetch)；其内 fetch 必须保留 raw fetch。
	return `
...
(function() {
		...
		try {
			...
			var result = entries.map(function(e) {
				return {
					...
					// ?? 而非 ||：order/depth/probability/displayIndex/group_weight 的 0 是 ST 合法值
					// （order=0 排最前、depth=0 紧贴生成位、probability=0 永不触发），|| 把 0 静默顶回默认
					// = 排序/注入位/概率全错（2026-07-15 falsy 扫描确诊）。|| 0 类字段默认即 0，无吞值，保持不动。
					order: e.order ?? 100,
	 ...
	 * 此为 ST 兼容契约默认值，供第三方 ST 脚本调用不报错，无对应可读真源，保留标注（凛倾 T12 判据：无对应概念的字段保留标注）。
	 * @returns {object}
	 */
