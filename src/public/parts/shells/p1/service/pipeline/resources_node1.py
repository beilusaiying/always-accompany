...
# 关联链（对照 JS 源）：
#   → p1_resdir.mjs getResDir/findResource（候选序 env P1_RESOURCE_DIR > memory/p1_res > 旧运行位）
...
def _derive_repo_root() -> Optional[Path]:
    try:
        return _THIS_DIR.parents[6]  # pipeline→service→p1→shells→parts→public→src→<repo>
    except IndexError:
        return None


def _resolve_memory_dir() -> Path:
    env = os.environ.get("P1_PY_MEMORY_DIR")
    if env:
        return Path(env)
    root = _derive_repo_root()
    if root is not None:
        cand = root / "src" / "yonban" / "core" / "functions" / "memory"
        if cand.is_dir():
            return cand
    return _JS_MEMORY_ABS


MEMORY_DIR: Path = _resolve_memory_dir()

# ── RES_DIR（p1_res 资源库）解析 [对照 p1_resdir.mjs:getResDir 候选序] ──
_RES_DEFAULT = MEMORY_DIR / "p1_res"
_RES_CANDIDATES: list[Path] = [
    p for p in (
...
def get_res_dir() -> Path:
    """[对照 p1_resdir.mjs:getResDir] 返回首个存在的候选；全缺返回默认并 warn（不抛，下游各 loader 自行退化）。"""
    global _res_dir
    if _res_dir is not None:
        return _res_dir
    _res_dir = _RES_DEFAULT
    for c in _RES_CANDIDATES:
        try:
            if c.exists():
                _res_dir = c
                break
    ...
    if not _res_dir.exists():
        print("[p1_res] P1 资源库未找到(env P1_RESOURCE_DIR / memory/p1_res / 旧运行位均无) → POS 词典/THUOCL 仲裁/停用词将退化为空", file=sys.stderr)
    return _res_dir


def find_resource(rel: str) -> Optional[Path]:
    """[对照 p1_resdir.mjs:findResource] 逐候选找相对路径，找到返回，全缺 None。"""
    for base in _RES_CANDIDATES:
        try:
            p = base / rel
            if p.exists():
                return p
...
def resolve_resource(filename: str) -> Optional[Path]:
    ...
    for p in (MEMORY_DIR / filename, MEMORY_DIR / "resources" / filename):
        if p.exists():
            return p
    return find_resource(filename)


# ── 用户插拔词库目录（0731 读写同源修复：原平移 JS 旧插件路径=读A写B——服务 saveUserVocab 写
#   config.vocabDir(service/data/vocab)，管线读旧插件 vocab/ → P9 写的词库永远不进召回。
#   收口：读侧同走 p1_config.vocabDir 单源；env 覆盖保留；p1_config 不可 import（直连测试
#   sys.path 无 service/）时兜底旧插件路径并保持 JS 对照语义。每次调用现解析=config 热改即生效。）──
def get_user_vocab_dir() -> Path:
    ...
    except Exception:  # noqa: BLE001 — 直连管线测试场景无 service 包，走旧路径兜底
        pass
    return _resolve_user_vocab_dir_legacy()


def _resolve_user_vocab_dir_legacy() -> Path:
    # [对照 p1_pool.mjs:_USER_VOCAB_DIR = <repo>/data/plugins/beilu-p1-selfdriven/vocab/]
    # 从 MEMORY_DIR 反推仓库根：memory→functions→core→yonban→src→<repo>
    try:
        repo = MEMORY_DIR.parents[4]
        return repo / "data" / "plugins" / "beilu-p1-selfdriven" / "vocab"
    except IndexError:
        return Path("data/plugins/beilu-p1-selfdriven/vocab")
