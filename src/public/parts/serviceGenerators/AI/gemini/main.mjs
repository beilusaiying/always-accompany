	...
	const result = {
		...
		StructCall: async (prompt_struct, options = {}) => {
			try {
				...
				// ================================================================
				// 司令员模式检测：beilu-preset 是否接管了提示词组装
				// ================================================================
...
` }]
				...
				if (config.disable_default_prompt || commanderMode) baseMessages.length = 0
...
`,
				...
				if (commanderMode) {
						...
						// 将预设/注入消息转换为 Gemini API 格式
						// role: system/user → 'user', assistant → 'model'
						...
						// 5 段结构：
						// beforeChat（头部预设，system only）
						// injectionAbove（@D>=1，聊天记录上方，可选 role）
						// chatHistory（聊天记录）
						// injectionBelow（@D=0，聊天记录下方，可选 role）
						// afterChat（尾部预设，system only）
						const prefixMessages = [
							...beforeChat.map(toGeminiMsg),
							...injectionAbove.map(toGeminiMsg),
						];
						const suffixMessages = [
							...injectionBelow.map(toGeminiMsg),
							...afterChat.map(toGeminiMsg),
						];
	
						// Token 限制处理
						const tokenLimit = presetModelParams.max_context || config.max_input_tokens;
						...
						if (totalEstimatedTokens < tokenLimit * 0.9) {
							finalMessages = [...prefixMessages, ...chatHistory, ...suffixMessages];
						} else {
					...
					if (presetModelParams.temperature !== undefined) configOverrides.temperature = presetModelParams.temperature;
					if (presetModelParams.top_p !== undefined) configOverrides.topP = presetModelParams.top_p;
					if (presetModelParams.top_k !== undefined) configOverrides.topK = presetModelParams.top_k;
					if (presetModelParams.max_tokens !== undefined) configOverrides.maxOutputTokens = presetModelParams.max_tokens;
					if (presetModelParams.frequency_penalty !== undefined) configOverrides.frequencyPenalty = presetModelParams.frequency_penalty;
					if (presetModelParams.presence_penalty !== undefined) configOverrides.presencePenalty = presetModelParams.presence_penalty;
					if (presetModelParams.seed !== undefined) configOverrides.seed = presetModelParams.seed;
