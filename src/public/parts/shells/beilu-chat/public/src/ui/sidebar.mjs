...
import { charList, personaName, pluginList, setCharList, setPersonaName, setPluginList, setWorldName, worldName } from '../chat.mjs'
import { addCharacter, addPlugin, removeCharacter, removePlugin, setPersona, setWorld, triggerCharacterReply } from '../endpoints.mjs'
...
async function renderWorldDetails(worldName) {
	...
	if (!cachedDom.world[worldName]) {
		...
		const worldCard = cachedDom.world[worldName] = await renderTemplate('world_info_chat_view', {
			...worldData.info,
			avatar: worldData.info?.avatar || 'https://api.iconify.design/mdi/earth.svg',
		})
...
async function renderCharDetails(charName, frequency_num) {
	...
	if (!cachedDom.character[charName]) {
		...
		const charCard = cachedDom.character[charName] = await renderTemplate('char_info_chat_view', {
			...charData.info,
			avatar: charData.info?.avatar || 'https://api.iconify.design/mdi/account-circle.svg',
			frequency_num
		})
		...
		// frequency slider is display-only (backend endpoint removed)
