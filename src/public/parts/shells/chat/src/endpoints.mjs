...
export function setEndpoints(router) {
	...
	router.post('/api/parts/shells\\:chat/:chatid/messages/delete-range', authenticate, async (req, res) => {
		try {
			...
			const result = await deleteMessagesRange(chatid, startIndex, endIndex)
	...
	router.get('/api/parts/shells\\:chat/:chatid/fake-send', authenticate, async (req, res) => {
		try {
			const { chatid } = req.params
			const result = await fakeSend(chatid)
			res.status(200).json(result)
		} catch (err) {
			res.status(500).json({ error: err.message || 'Failed to build fake-send' })
		}
	})
