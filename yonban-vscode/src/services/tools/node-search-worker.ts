 ...
 * 不访问 VS Code、snapshot、WebSocket 或其他 extension-host 状态。
 */
...
interface NodeSearchJob {
  jobId: string;
  kind: "content" | "name" | "content_metadata" | "name_metadata";
  absPath: string;
  wsRoot: string;
  pattern: string;
  filePattern?: string;
...
interface WorkerResult {
  records: ContentMatch[] | NameMatch[] | MetadataMatch[];
  complete: boolean;
  workerReason: string;
...
const job = (workerData && typeof workerData === "object" ? workerData : {}) as NodeSearchJob;
const jobId = typeof job.jobId === "string" ? job.jobId : "";
let cancelRequested = false;

port.on("message", (message: { type?: string; jobId?: string }) => {
...
function stopReason(count: number): string | undefined {
...
function searchContent(): WorkerResult {
  let regex: RegExp;
  try {
  ...
  let rangeLimitReason: string | undefined;
  ...
  const matchesFilePattern = (file: string): boolean => {
  ...
  return {
    ...
    workerReason,
    ...(rangeLimitReason ? { rangeLimitReason } : {}),
  };
}

function searchNames(): WorkerResult {
 ...
 */
function collectContentMetadata(): WorkerResult {
  const records: Array<{ path: string; mtime: number }> = [];
  for (const item of Array.isArray(job.records) ? job.records : []) {
    const reason = stopReason(-1);
    if (reason) return { records: [], complete: false, workerReason: reason };
  ...
  return { records, complete: true, workerReason: "node_completed" };
}

function collectNameMetadata(): WorkerResult {
  const records: NameMatch[] = [];
  for (const item of Array.isArray(job.records) ? job.records : []) {
    const reason = stopReason(-1);
    if (reason) return { records: [], complete: false, workerReason: reason };
    const relativePath = typeof item?.path === "string" ? item.path : "";
