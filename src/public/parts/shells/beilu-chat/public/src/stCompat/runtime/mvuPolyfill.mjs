...
(function() {
	...
	window.Mvu = {
		...
		getMvuData: function(options) {
			/* ★ 修复：优先从 SillyTavern.chat[0].variables[0] 读取
			   前端 getMvuVariablesForRendering() 已做深度合并累积，
			   结果注入到 SillyTavern.chat[0].variables[0]，
			   这是最完整最权威的变量来源 */
			var stChat = (window.SillyTavern && window.SillyTavern.chat) ? window.SillyTavern.chat : [];
			if (stChat.length > 0 && stChat[0] && stChat[0].variables) {
				var swipeId = stChat[0].swipe_id || 0;
				var injected = stChat[0].variables[swipeId];
				if (injected && typeof injected === 'object' && Object.keys(injected).length > 0) {
					if (!injected.stat_data) injected.stat_data = {};
					if (!injected.initialized_lorebooks) injected.initialized_lorebooks = {};
					return injected;
				}
			}
			/* 回退到 getVariables（从 __beiluVarStore 读取） */
