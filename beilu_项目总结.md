...
> 版本: v1.1
> 日期: 2026-02-23
> 状态: 1.1 版 — 新增智能预设切换系统，实现永久记忆 + 多模式自适应 COT
...
| **智能预设切换** | P1 检索 AI 实时分析上下文，自动切换最适合的提示词预设，适配多种交互模式 |
...
| ID  | 名称             | 触发              | 用途                                    |
| --- | ---------------- | ----------------- | --------------------------------------- |
| P1  | 检索 AI          | auto_on_message   | 多轮深度检索（最多 5 轮）+ 智能预设切换 |
...
P1 检索 AI 在每轮对话中不仅执行记忆检索，还会**实时分析对话上下文**，判断当前场景是否需要切换到更适合的提示词预设。这意味着：

- **自动适配多种模式**：日常闲聊 → 角色扮演预设；用户说"帮我写代码" → 自动切换工作预设；讨论完毕 → 自动切回
- **提示词 + COT 随模式变化**：不同预设携带不同的思维链（COT）和行为规则，切换预设 = 切换整套 AI 行为模式
- **冷却机制防振荡**：内置冷却计数器，防止同一预设被频繁重复切换
- **纯提示词驱动**：P1 的切换判断完全由提示词中的 COT 指导，用户可自定义切换策略
- **手动快速切换**：用户也可在聊天界面手动一键切换预设

**工作流程**：
```
用户发送消息 → P1 运行（检索 + 分析）→ 输出 <presetSwitch>目标预设</presetSwitch>
             → 系统自动执行切换 → 当前轮聊天 AI 使用新预设回复
```

这是记忆系统的重大突破——AI 不仅能记住过去，还能**根据当下的对话意图，实时调整自己的行为模式**，实现真正的多模式自适应。
| P6  | 格式检查/修复 AI | manual_button     | 表格和记忆文件修复        |
...
- ✅ Phase 6：智能预设切换系统（P1 自动切换 + 冷却机制 + 手动快速切换 + beilu-home 管理 UI）
...
| 20  | P1 智能预设切换 + 冷却机制                  | 实时适配多交互模式，防止切换振荡            |
| 21  | 预设切换纯提示词驱动                        | 切换逻辑由 COT 指导，零硬编码，用户可自定义 |
...
| 智能预设切换                   | ✅    | 已完成（P1 自动切换 + 冷却机制 + 多模式自适应 COT） |
...
*此文档综合了 beilu-always accompany 项目截至 2026-02-23 的所有工作日志、设计文档和代码分析。如需详细了解某个模块，请参考对应的工作日志文件。*
