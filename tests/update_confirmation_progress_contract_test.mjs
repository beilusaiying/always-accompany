...
assert.doesNotMatch(update, /STARTUP_CHECK_DELAY_MS|setTimeout\(\(\) => \{[\s\S]*detectRemoteUpdate\(\)/, 'runtime must not schedule startup update checks')
...
assert.match(psLauncher, /function Invoke-StartupUpdate/, 'PowerShell launcher must own startup update checks')
assert.match(psLauncher, /git -C \$PROJECT_DIR pull --ff-only origin main/, 'PowerShell launcher must fast-forward before server start')
assert.match(psLauncher, /Invoke-StartupUpdate/, 'startup update check must run before server start')
