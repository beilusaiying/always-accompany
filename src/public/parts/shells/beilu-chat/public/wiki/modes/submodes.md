...
API 源和模型绑定都是可选项：子模式未填写时跟随当前全局有效配置；填写后仅覆盖该子模式。全局已经保存一个 API 源，并不等于每个子模式都已获得独立绑定——需要固定工作流用的源/模型时，应在相应子模式的编辑页显式选择并保存。

新建子模式的执行默认值只用于播种新条目，不会覆盖已有用户配置：提示词后处理为 `strict`，尾部预填充关闭，最大上下文为 `1000000` token，最大输出为 `30000` token。它们仍可在子模式编辑页改动；不要把这些默认值当成运行时强制硬编码。
...
- **先检查绑定**：某一子模式没有回复或用错模型时，先确认它是“继承全局”还是已显式绑定 API 源/模型，再检查服务源本身

## 相关页面

- [AI 服务源配置](beilu:wiki/ai-service/api-config.md) — 创建与管理全局服务源
- [模型参数](beilu:wiki/ai-service/model-params.md) — 全局与子模式参数的优先关系
- [消息管线](beilu:wiki/developer/message-pipeline.md) — 可配置提示词文本的运行时边界
