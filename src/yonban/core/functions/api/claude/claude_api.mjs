/**
 * Claude（逆向） — 逆向 claude.ai 网页 API
 * API端点：https://api.claude.ai（Buffer 混淆编码，运行时解码）
 * 认证：Cookie 轮换（cookie_array 多账号，failedCookies 集合跳过失败项）
 * ⚠ 逆向 API：端点/请求头/对话管理均模拟浏览器行为（User-Agent/sec-ch-ua/Referer 伪造），
 *   claude.ai 接口变更将直接导致本模块失效；ClewdSimulation 做 impersonation 检测与截断。
 *   本文件只提供 ClaudeAPI 类，由同目录 main.mjs 包装为 beilu AIsource_t 接口。
 */
// claude_api.mjs
...
// SEC-F4（红方 round2，多用户）：server 部署下用户自配 r_proxy 反代不可指内网。
...
async function checkResErr(res, throwIt = true) {
	...
	if (res.status < 200 || res.status >= 300) {
		wbD(null, 'ai:claude', 'checkResErr:non2xx', false, 'Claude API 非2xx响应', { status: res.status })
		...
		try {
			...
			wbD(null, 'ai:claude', 'checkResErr:unparseable', false, 'Claude API 错误响应无法解析为JSON', { status: res.status })
		...
		if (errAPI) {
			...
			// Claude 特有错误结构告警：token超限 / 限流 / 鉴权
			wbD(null, 'ai:claude', 'checkResErr:apiError', false, 'Claude API 结构化错误', { status: res.status, type: errAPI.type, tokenExceeded: errAPI.type === 'invalid_request_error', rateLimited: res.status === 429, authFailed: res.status === 401 || res.status === 403 })
...
export class ClaudeAPI {
	...
	async firstLogin() {
		...
		while (attempts < maxAttempts) try {
			...
			await assertSafeOutboundInServerMode(rProxy) // SEC-F4：server 下拒内网反代目标

			const orgsResponse = await fetch(`${rProxy}/api/organizations`, {
				method: 'GET',
				headers,
				signal: AbortSignal.timeout(30000), // 冷路径超时（auth setup GET，非流式）防服务器不回头永挂
			})
			...
			wbT(null, 'ai:claude', 'firstLogin:ok', { uuidOrg: this.uuidOrg })
			console.log(`Logged in to Claude API. Organization UUID: ${this.uuidOrg}`)
			return // 成功获取到 uuidOrg，直接返回
		} catch (error) {
			wbD(null, 'ai:claude', 'firstLogin:attemptFailed', false, 'firstLogin尝试失败', { attempt: attempts + 1, status: error?.status, message: String(error?.message || error).slice(0, 200) })
	...
	shouldRenewChat(messages) {
		const currentPrompt = {
			...
			// 占位符排除条件已删（凛倾0712：占位符 producer 全删，条件成死引用）
			lastSystem: messages.findLast(m => m.role === 'system'),
		}
	...
	async callClaudeAPI(messages, model) {
		wbT(null, 'ai:claude', 'callClaudeAPI:enter', { model: model || this.config.model, msgCount: messages?.length ?? 0 })
		if (!wbD(null, 'ai:claude', 'callClaudeAPI:cookies', !!this.config.cookie_array?.length, '未配置Claude cookie', { count: this.config.cookie_array?.length ?? 0 }))
		...
		if (!this.uuidOrg) {
			...
			if (!wbD(null, 'ai:claude', 'callClaudeAPI:uuidOrg', !!this.uuidOrg, 'firstLogin后仍无组织UUID', null))
		...
		wbT(null, 'ai:claude', 'callClaudeAPI:renew', { shouldRenew })
		if (shouldRenew) {
		...
		while (attempt < this.config.cookie_array.length + 1) try {
			...
			wbT(null, 'ai:claude', 'callClaudeAPI:payload', { attempt, model: payload.completion.model, promptLen: prompt.length, convUuid: this.conversationUuid })

			wbT(null, 'ai:claude', 'callClaudeAPI:request:before', { attempt })
			// 连接/响应头超时：原 fetch 无超时，服务器接受连接却不回响应头时永挂。独立 controller 限时，
			// 拿到响应头即 clearTimeout（body=response.json() 由后续读取，headers-hang 是主风险）。
			// T15-3：超时值读 config.timeout（configTemplate 默认 120000 单源）；缺值/非正数=现行为 120000。
			const _claudeTimeoutMs = this.config.timeout > 0 ? this.config.timeout : 120000
			const _claudeCtrl = new AbortController()
			const _claudeTimer = setTimeout(() => _claudeCtrl.abort(new Error(`[claude] 等待响应头超时（${_claudeTimeoutMs}ms 无响应）`)), _claudeTimeoutMs)
			const response = await fetch(`${rProxy}/api/organizations/${this.uuidOrg}/chat_conversations/${this.conversationUuid}/completion`, {
				...
				signal: _claudeCtrl.signal,
			}).finally(() => clearTimeout(_claudeTimer))
			wbT(null, 'ai:claude', 'callClaudeAPI:request:after', { attempt, status: response.status })
			...
			if (wbD(null, 'ai:claude', 'callClaudeAPI:completion', !!responseJSON.completion, 'Claude API 返回空completion', { impersonated: claudeSim.impersonated, len: responseJSON.completion?.length ?? 0 }))
				return responseJSON.completion
			else
				throw new Error('Claude API error: empty response')
		} catch (error) {
			wbD(null, 'ai:claude', 'callClaudeAPI:catch', false, 'Claude API 调用异常', { attempt, status: error?.status, type: error?.type, exceeded_limit: error?.exceeded_limit, message: String(error?.message || error).slice(0, 200) })
		...
		wbD(null, 'ai:claude', 'callClaudeAPI:allCookiesFailed', false, '所有cookie均已失败', { cookieCount: this.config.cookie_array?.length ?? 0 })
