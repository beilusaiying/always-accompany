...
export default {
  ...
  Load: async ({ router }) => {
    ...
    // ============================================================
    router.get(
      "/api/parts/shells\\:beilu-home/network-info",
      async (_req, res) => {
        try {
            ...
            try {
              ...
              if (fs.existsSync(wbConfigPath)) {
                ...
                if (Array.isArray(wbData.worldbooks)) {
                  ...
                  wbData.worldbooks = wbData.worldbooks.filter(
                    (wb) => wb.boundCharName !== charName,
                  );
                  ...
                  if (removed > 0) {
                    ...
                    console.log(
                      `[beilu-home] 已清理角色 "${charName}" 绑定的 ${removed} 个世界书`,
                    );
                  }
                ...
                for (const file of chatFiles) {
                  ...
                  if (!chatIdsToDelete.includes(chatid)) {
                    ...
                    try {
                      ...
                      console.log(
                        `[beilu-home] fallback 删除聊天文件: ${file}`,
                      );
                    } catch (e) {
                      console.warn(
                        `[beilu-home] 删除聊天文件 ${file} 失败:`,
                        e.message,
                      );
                    }
                  }
    ...
    router.post(
      "/api/parts/shells\\:beilu-home/diag/clear-snapshots",
      (_req, res) => {
        try {
    ...
    router.post(
      "/api/parts/shells\\:beilu-home/diag/clear-logs",
      (_req, res) => {
        try {
