...
export function buildCompatMessages(mergedChatLog,ignoreFiles,username,attachmentHistory="hide",attachmentTexts){
  ...
  }else if(file.mime_type.startsWith('audio/')){const fmt={'audio/wav':'wav','audio/wave':'wav','audio/x-wav':'wav','audio/mpeg':'mp3','audio/mp3':'mp3','audio/mp4':'mp4','audio/m4a':'m4a','audio/webm':'webm','audio/ogg':'webm'};const format=fmt[file.mime_type.toLowerCase()]||'wav';const audioData=Buffer.isBuffer(file.buffer)?file.buffer.toString('base64'):typeof file.buffer==='string'&&file.buffer.startsWith('file:')?_resolveFileHash(file.buffer,username)?.toString('base64'):typeof file.buffer==='string'?file.buffer:'';if(audioData)contentParts.push({type:'input_audio',input_audio:{data:audioData,format}});}}if(contentParts.length>1)message.content=contentParts;}return message;});}
