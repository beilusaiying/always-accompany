...
export function initializeMessageInput() {
  ...
  // photo-button 已在 HTML 中移除（替换为 single-inject-btn），跳过绑定
...
async function toggleVoiceRecording() {
  if (isRecording) {
    ...
    voiceButtonElement.innerHTML = await fetch(
      "https://api.iconify.design/material-symbols/mic.svg",
    ).then((res) => res.text());
    while (isRecording) {
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        isRecording = await handleFilesSelect(
          fakeEvent,
          SelectedFiles,
          attachmentPreviewContainer,
        );
      ...
      mediaRecorder.start();
      voiceButtonElement.innerHTML = await fetch(
        "https://api.iconify.design/line-md/square.svg",
      ).then((res) => res.text());
      isRecording = true;
    } catch (error) {
...
async function sendMessage() {
  ...
  try {
    ...
    messageInputElement.style.height = "auto"; // 发送后重置高度
