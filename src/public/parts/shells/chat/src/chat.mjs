...
/**
 * 模拟发送：构建完整的 prompt_struct 并转换为 messages 格式，
 * 但不真正调用 AI。用于提示词查看器。
 *
 * 返回的 messages 数组反映 AI 实际会收到的完整消息序列：
 * - commanderMode: 预设系统条目 + 聊天记录（含 injection depth 注入） + 模型参数
 * - fallback: 系统提示词 + 合并后的聊天记录
 *
 * @param {string} chatid - 聊天ID。
 * @returns {Promise<object>} 包含 messages 数组和元信息的对象。
 */
export async function fakeSend(chatid) {
	const chatMetadata = await loadChat(chatid)
	if (!chatMetadata) throw new Error('Chat not found')
	...
	// 找到第一个角色作为目标
	const charnames = Object.keys(timeSlice.chars)
	...
	// 向后兼容：如果没有 before/after 但有 messages，全部作为 before
	const hasCommanderData = beforeChat || presetExt?.beilu_preset_messages

	if (hasCommanderData) {
		// ============================================================
		// commanderMode: 5 段式消息序列
		// 头部预设 → 注入上方 → 聊天记录 → 注入下方 → 尾部预设
		// ============================================================

		const effectiveBefore = beforeChat || presetExt?.beilu_preset_messages || []
		...
		const toMsg = (m, section, source = 'preset') => ({
			...
			_source: source,
			_section: section,
			_identifier: m.identifier || undefined,
			_is_marker: m.is_marker || false,
		})

		// 1. 头部预设条目（beforeChat）
		const beforeSection = effectiveBefore.map(m => toMsg(m, 'beforeChat'))
		...
		const chatSection = chatLog.map(entry => ({
		...
		// 完整消息序列
		const fullMessages = [...beforeSection, ...aboveSection, ...chatSection, ...belowSection, ...afterSection]

		// 统计
		const allPreset = [...beforeSection, ...afterSection]
		const allInjection = [...aboveSection, ...belowSection]
		const totalChars = fullMessages.reduce((sum, m) => sum + (m.content?.length || 0), 0)
		const systemChars = allPreset.reduce((sum, m) => sum + (m.content?.length || 0), 0)
		const chatChars = chatSection.reduce((sum, m) => sum + (m.content?.length || 0), 0)
		const injectionChars = allInjection.reduce((sum, m) => sum + (m.content?.length || 0), 0)

		return {
			messages: fullMessages,
			_meta: {
				source: 'beilu-preset-commander',
				charname,
				commander_mode: true,
				chat_log_count: chatMetadata.chatLog.length,
				// 各区段统计
				before_preset_count: beforeSection.filter(m => !m._is_marker).length,
				after_preset_count: afterSection.filter(m => !m._is_marker).length,
				preset_entry_count: allPreset.filter(m => !m._is_marker).length,
				chat_message_count: chatSection.length,
				injection_above_count: aboveSection.length,
				injection_below_count: belowSection.length,
				injection_count: allInjection.length,
				// 字符数
				total_chars: totalChars,
				system_chars: systemChars,
				chat_chars: chatChars,
				injection_chars: injectionChars,
	...
	// ============================================================
	// fallback: 无 commanderMode，使用标准 prompt_struct
	// ============================================================
	...
	if (systemPrompt) {
		messages.push({
			...
			_source: 'prompt_struct',
		})
	}

	for (const entry of mergedChatLog) {
		messages.push({
			role: entry.role === 'user' ? 'user' : 'assistant',
			content: entry.content || '',
			name: entry.name || undefined,
	...
	const systemChars = messages.filter(m => m.role === 'system').reduce((sum, m) => sum + (m.content?.length || 0), 0)

	return {
		messages,
		_meta: {
			source: 'prompt_struct',
			charname,
