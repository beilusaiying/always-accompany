# PowerShell 参数补全脚本，用于 beilu 的 'discordbot' shell。
#
# 使用方法:
#   beilu run shells <username> discordbot <action> [args...]
#
# 支持的 Action:
		...
		1 {
			...
			switch ($action) {
				...
				"get-template" {
					...
					Get-BeiluPartList -parttype chars -Username $Username | Where-Object { $_.StartsWith($WordToComplete) }
					break
				}
				# 'list' 命令没有第二个参数。
