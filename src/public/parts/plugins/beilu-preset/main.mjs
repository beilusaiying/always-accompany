    ...
    try {
      ...
      if (saved) {
        ...
        if (activeData?.preset_json) {
          ...
          // 同步独立修改过的模型参数
          if (
            activeData.model_params &&
            Object.keys(activeData.model_params).length > 0
          ) {
          ...
          console.log(
            `[beilu-preset] 预设已恢复: "${engine.presetName}" (${engine.promptEntries.size} 条目)`,
          );
        } else {
        ...
        try {
          if (req.body) {
            if (req.body.context_msg_limit !== undefined) {
              runtimeParams.context_msg_limit =
                parseInt(req.body.context_msg_limit, 10) || 0;
            }
            ...
            if (req.body.prompt_post_processing !== undefined) {
              ...
              runtimeParams.prompt_post_processing = valid.includes(
                req.body.prompt_post_processing,
              )
                ? req.body.prompt_post_processing
                : "none";
            }
            ...
            if (req.body.claude_prefill_enabled !== undefined) {
              runtimeParams.claude_prefill_enabled =
                !!req.body.claude_prefill_enabled;
            }
      ...
      SetData: async (data) => {
        ...
        if (data.switch_preset) {
          ...
          if (presetData?.preset_json) {
            ...
            if (
              presetData.model_params &&
              Object.keys(presetData.model_params).length > 0
            ) {
          ...
          if (configData.presets[name]) {
            ...
            if (name === configData.active_preset) {
              ...
              if (remaining.length > 0) {
                ...
                if (
                  nextData.model_params &&
                  Object.keys(nextData.model_params).length > 0
                ) {
        ...
        if (data.add_entry) {
          ...
          if (!entryData.identifier) {
            ...
            if (ok) {
              syncActivePresetToConfig();
              console.log(
                `[beilu-preset] 条目已新增: "${entryData.identifier}"`,
              );
            } else {
              console.warn(
      ...
      TweakPrompt: (arg, prompt_struct, my_prompt, detail_level) => {
        ...
        if (detail_level === 2) {
          ...
          if (memoryExt?.preset_switch_to) {
            ...
            if (
              targetData?.preset_json &&
              targetName !== configData.active_preset
            ) {
              // 保存当前预设状态
              syncActivePresetToConfig();
              // 加载新预设到引擎
              ...
              if (
                targetData.model_params &&
                Object.keys(targetData.model_params).length > 0
              ) {
              ...
              console.warn(
                `[beilu-preset] P1 请求切换到不存在的预设: "${targetName}"`,
              );
            }
          }
          ...
          // [DIAG] 诊断点5：Round 1 - plugin_prompts 所有 key
          console.log(
            `[beilu-preset][DIAG] Round 1: plugin_prompts keys:`,
          ...
          for (const [name, prompt] of Object.entries(
            ...
            // [DIAG] 诊断点6：Round 1 - 每个插件的 flattenPromptTexts 结果
            if (name === "beilu-memory") {
              console.log(
                `[beilu-preset][DIAG] Round 1: beilu-memory flattenPromptTexts → ${pluginContent.length}字符, truthy=${!!pluginContent}`,
              );
              console.log(
                `[beilu-preset][DIAG] Round 1: beilu-memory prompt.text 原始:`,
                JSON.stringify(
                  prompt?.text?.map((t) => ({
              ...
              console.log(
                `[beilu-preset][DIAG] Round 1: beilu-memory extension keys:`,
                Object.keys(prompt?.extension || {}).join(", "),
              );
              const mdi = prompt?.extension?.memory_depth_injections;
              console.log(
                `[beilu-preset][DIAG] Round 1: beilu-memory memory_depth_injections: ${mdi ? mdi.length + "条" : "undefined"}`,
              );
              if (mdi && mdi.length > 0) {
                console.log(
                  `[beilu-preset][DIAG] Round 1: memory_depth_injections 详情:`,
                  mdi
                    .map(
                      (d) =>
              ...
              console.log(
                `[beilu-preset][DIAG] Round 1: plugin "${name}" flattenPromptTexts → ${pluginContent.length}字符`,
              );
            }
          ...
          for (const [name, prompt] of Object.entries(
            ...
            if (prompt) {
              ...
              // [DIAG] 诊断点7：Round 1 - 清空后 extension 是否保留
              if (name === "beilu-memory") {
                console.log(
                  `[beilu-preset][DIAG] Round 1: 清空 beilu-memory text 后, extension存在=${!!prompt?.extension}, memory_depth_injections存在=${!!prompt?.extension?.memory_depth_injections}, 长度=${prompt?.extension?.memory_depth_injections?.length ?? "N/A"}`,
                );
              }
            }
          }

          // [DIAG] 诊断点8：Round 1 完成 - env 汇总
          {
            const pluginKeys = Object.keys(env).filter((k) =>
              k.startsWith("plugin_"),
            );
            console.log(
              `[beilu-preset][DIAG] Round 1 完成: env 中 ${pluginKeys.length} 个 plugin_* 键:`,
              pluginKeys
                .map(
                  (k) =>
            ...
            console.log(
              `[beilu-preset][DIAG] Round 1 完成: env.char_prompt=${(env.char_prompt || "").length}字符, env.user_prompt=${(env.user_prompt || "").length}字符, env.world_prompt=${(env.world_prompt || "").length}字符`,
            );
          }
        ...
        if (detail_level === 1) {
          ...
          if (
            runtimeParams.context_msg_limit > 0 &&
            chatLog.length > runtimeParams.context_msg_limit
          ) {
            chatLog = chatLog.slice(-runtimeParams.context_msg_limit);
            prompt_struct.chat_log = chatLog;
            console.log(
              `[beilu-preset] 上下文屏蔽: 保留最近 ${runtimeParams.context_msg_limit} 条消息（原 ${prompt_struct.chat_log?.length || 0} 条）`,
            );
          }
          ...
          const { beforeChat, afterChat, injectionAbove, injectionBelow } =
            engine.buildAllEntries(env, macroMemory, chatLog);
          ...
          // [DIAG] 诊断点9：Round 2 - 遍历 env 中所有 plugin_* 键的状态
          {
            const allPluginKeys = Object.keys(env).filter((k) =>
              k.startsWith("plugin_"),
            );
            console.log(
              `[beilu-preset][DIAG] Round 2: env 中 plugin_* 键总数=${allPluginKeys.length}`,
            );
            for (const pk of allPluginKeys) {
              const pv = env[pk];
              console.log(
                `[beilu-preset][DIAG] Round 2: ${pk} → len=${(pv || "").length}, truthy=${!!pv}, trimNonEmpty=${!!pv?.trim()}, willEnterIf=${!!(pv && pv.trim())}`,
              );
            }
            // 特别检查 beilu-memory 不在 env 中的情况
            if (!allPluginKeys.includes("plugin_beilu-memory")) {
              console.log(
                `[beilu-preset][DIAG] Round 2: ⚠️ env 中没有 plugin_beilu-memory 键！检查 prompt_struct.plugin_prompts 中是否有 beilu-memory...`,
              );
              ...
              console.log(
                `[beilu-preset][DIAG] Round 2: prompt_struct 中 beilu-memory 存在=${hasInPS}, memory_depth_injections=${mdi ? mdi.length + "条" : "undefined"}`,
              );
            }
          }
          ...
          for (const [key, value] of Object.entries(env)) {
            if (key.startsWith("plugin_") && value && value.trim()) {
              ...
              const memoryDepthInjections =
                pluginPrompt?.extension?.memory_depth_injections;

              // [DIAG] 诊断点10：Round 2 - beilu-memory 分支判断细节
              if (pluginName === "beilu-memory") {
                console.log(
                  `[beilu-preset][DIAG] Round 2: beilu-memory 进入外层if, pluginPrompt存在=${!!pluginPrompt}, memoryDepthInjections=${memoryDepthInjections ? memoryDepthInjections.length + "条" : "undefined"}`,
                );
                console.log(
                  `[beilu-preset][DIAG] Round 2: beilu-memory 将进入depth处理=${!!(memoryDepthInjections && memoryDepthInjections.length > 0)}`,
                );
              }

              if (
                pluginName === "beilu-memory" &&
                memoryDepthInjections &&
                memoryDepthInjections.length > 0
              ) {
                // beilu-memory 提供了带 depth 的注入条目
                // 按 depth 分配到 injectionAbove (depth>=1) 或 injectionBelow (depth=0)
                // 先按 order 排序
                const sorted = [...memoryDepthInjections].sort(
                ...
                for (const depthInj of sorted) {
                  ...
                  // [DIAG] 诊断点11：Round 2 - 注入推入
                  console.log(
                    `[beilu-preset][DIAG] Round 2: 推入 memory_${depthInj.id} → depth=${depthInj.depth}, target=${depthInj.depth >= 1 ? "injectionAbove" : "injectionBelow"}, content=${injContent.length}字符`,
                  );
                  if (depthInj.depth >= 1) {
                    injectionAbove.push(msg);
                  } else {
          ...
          // [DIAG] 诊断点12：Round 2 完成汇总
          console.log(
            `[beilu-preset][DIAG] Round 2 完成: beforeChat=${beforeChat.length}, afterChat=${afterChat.length}, injectionAbove=${injectionAbove.length}, injectionBelow=${injectionBelow.length}`,
          );
          if (injectionAbove.length > 0) {
            console.log(
              `[beilu-preset][DIAG] Round 2 injectionAbove:`,
              injectionAbove
                .map(
                  (m) =>
          ...
          if (injectionBelow.length > 0) {
            console.log(
              `[beilu-preset][DIAG] Round 2 injectionBelow:`,
              injectionBelow
                .map(
                  (m) =>
                    `${m.identifier || m.name}(${m.content?.length || 0}字符,depth=${m.depth ?? "?"})`,
