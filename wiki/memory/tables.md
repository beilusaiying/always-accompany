...
表格跟着**角色卡**走：同一张角色卡下的所有对话窗口共享同一套表格——这个窗口里 AI 记下的内容，换个对话窗口照样在。
...
- **新增**（insertRow）：新增一行记录
- **修改**（updateRow）：更新已有记录
- **删除**（deleteRow）：直接删除记录，不留档
- **归档**（archiveRows）：把行移入热层的归档文件——与删除不同，归档的行之后可以恢复

具体的写入格式由记忆系统的 INJ 指令注入给 AI——INJ-1 会告诉 AI 当前模式下可用哪些表、每张表存什么、用什么语法写入，你不需要手动教。

写坏了也有兜底：如果 AI 的表格写入不符合格式，系统会把失败反馈带回下一轮，AI 会自己发现并重试——这是一个自愈闭环。
...
表格条目按阈值和日终流程自动从 hot 搬迁到 warm，再到 cold。搬迁由记忆系统的归档管线自动执行，详见[记忆归档与检索](archival.md)。
