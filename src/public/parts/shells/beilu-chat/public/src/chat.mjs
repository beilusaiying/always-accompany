...
export async function initializeChat() {
  ...
  console.log("[beilu-chat][DIAG] currentChatId:", currentChatId);
  if (currentChatId) {
    try {
      console.log(
        ...
        if (fallbackCount >= FALLBACK_MAX) {
          ...
          try {
            ...
            if (res.ok) {
              ...
              // 过滤掉当前失败的 chatid，避免回退到同一个无效聊天
              const validChats = allChats.filter(
                (c) => c.chatid !== currentChatId,
              );
              if (validChats.length > 0) {
                const fallbackChatId = validChats[0].chatid;
                console.log(
                  `[beilu-chat] 回退到最近聊天: ${fallbackChatId}（第${fallbackCount + 1}次）`,
                );
            ...
            console.warn("[beilu-chat] 回退查询失败:", fallbackErr.message);
          }
          // 回退也失败，清空 hash 进入空态
          sessionStorage.removeItem(FALLBACK_KEY);
          window.location.hash = "";
    ...
    console.log(
      "[beilu-chat] 没有活跃的聊天（无 chatId），跳过数据加载，等待用户选择聊天",
    );
  }
  ...
  // P0：初始化成功时清除回退计数
  sessionStorage.removeItem("beilu_chat_fallback_count");
