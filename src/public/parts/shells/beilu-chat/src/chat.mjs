...
class chatMetadata_t {
  ...
  static async fromJSON(json) {
    ...
    return Object.assign(new chatMetadata_t(), {
      ...
      LastTimeSlice: chatLog.length
        ? chatLog[chatLog.length - 1].timeSlice
        : new timeSlice_t(),
    });
  }
...
async function getChatRequest(chatid, charname, options = {}) {
  ...
  // 这解决了"旧聊天 timeSlice.plugins 中没有新注册的插件（如 beilu-browser）导致 GetPrompt 不被调用"的问题
 ...
 * 酒馆格式角色卡导入时可能残留这些宏，不清理会导致前端请求无效 URL 产生 404
 * @param {string|undefined} avatar - 原始 avatar 值
 * @returns {string|undefined} 清理后的 avatar 值，包含宏的返回 undefined
 */
...
async function BuildChatLogEntryFromCharReply(
  result,
  new_timeSlice,
  char,
  charname,
...
async function BuildChatLogEntryFromUserMessage(
  result,
  new_timeSlice,
  user,
  personaname,
      ...
      for (const [pluginName, plugin] of Object.entries(timeSlice.plugins)) {
        if (plugin?.interfaces?.chat?.ReplyHandler) {
          try {
            ...
            console.warn(
              `[chat] Plugin ${pluginName} ReplyHandler error:`,
              err.message,
            );
          }
        }
...
export async function modifyTimeLine(chatid, delta, absoluteIndex) {
  ...
  if (newTimeLineIndex >= chatMetadata.timeLines.length) {
    ...
    broadcastChatEvent(chatid, {
      type: "message_replaced",
      payload: {
        index: chatMetadata.chatLog.length - 1,
...
export async function deleteMessagesRange(chatid, startIndex, endIndex) {
  ...
  const end =
    endIndex != null
      ? Math.min(chatMetadata.chatLog.length, endIndex)
      : chatMetadata.chatLog.length;
  ...
  console.log(
    `[chat] 批量删除消息: chatid=${chatid}, range=[${start}, ${end}), deleted=${deletedCount}`,
  );
...
export async function deleteMessage(chatid, index) {
  ...
  if (chatMetadata.chatLog.length) chatMetadata.LastTimeSlice = last.timeSlice;
  else chatMetadata.LastTimeSlice = new timeSlice_t();
...
export async function editMessage(chatid, index, new_content) {
  ...
  const timeSlice =
    chatMetadata.chatLog[index].timeSlice ||
...
events.on("AfterUserDeleted", async (payload) => {
  ...
  for (const [chatId, data] of chatMetadatas.entries())
    if (data.username === username) chatIdsToDeleteFromCache.push(chatId);
...
export async function buildFakeSendRequest(chatid, charname) {
  ...
  if (commanderMode) {
    ...
    // 3. 聊天记录
    const chatLogMsgs = (prompt_struct.chat_log || []).map((e) => ({
      role:
        e.role === "user"
          ? "user"
          : e.role === "system"
    ...
    const isMutiChar =
      new Set(prompt_struct.chat_log.map((e) => e.name).filter(Boolean)).size >
      2;
    if (isMutiChar) {
      messages.push({
        role: "system",
