...
import { generateBeiluDebugScript } from "./runtime/beiluDebug.mjs";
import { generateBeiluHelperScript } from "./runtime/beiluHelper.mjs";
...
const CDN_URLS = {
  ...
  yaml: "/vendor/js-yaml.min.js",
  vue: "/vendor/vue.global.prod.js",
  ejs: "/vendor/ejs.min.js",
  "mvu-bundle":
...
/** localStorage key */
const STORAGE_KEY = KEYS.BEILU_ST_COMPAT_ENABLED;
...
export function detectNeeds(htmlContent) {
  ...
  let needsST =
    /\bTavernHelper\b|\bgetVariables\b|\beventOn\b|\bwaitGlobalInitialized\b|\bSillyTavern\.\w+\b|\binitializeGlobal\b|\beventEmit\b|\bgetAllVariables\b|\breplaceVariables\b|\berrorCatched\b|\beventOnce\b|\btavern_events\b|\biframe_events\b/.test(
      htmlContent,
    );
  ...
  let needsMVU =
    needsST &&
    /\bMvu\b|\bMagVarUpdate\b|\bstat_data\b|\bgetMvuData\b|\breplaceMvuData\b|\bmag_variable_/.test(
      htmlContent,
    );

  // ★ URL / Zod 模式检测：
  // 消息 HTML 中可能包含 <script src="...bundle.js"> 等外部引用，
  // 或者代码中直接使用 z.object() / z.string() 等 Zod API。
  // 这些依赖在 HTML 字符串中的关键字可能不匹配 MVU 层的标准检测，
  // 通过额外模式识别来确保 Zod CDN 被注入。
  if (!needsMVU) {
    const hasMvuUrls =
      /bundle\.js|mvu_zod\.js|MagVarUpdate|MagicalAstrogy|tavern_resource\/dist/.test(
        htmlContent,
      );
    const hasZodUsage =
      /\bz\.object\s*\(|\bz\.string\s*\(|\bz\.number\s*\(|\bz\.boolean\s*\(|\bz\.array\s*\(|\bz\.enum\s*\(/.test(
        htmlContent,
      );
    if (hasMvuUrls || hasZodUsage) {
      needsST = true;
      needsMVU = true;
      diag.debug(
        hasMvuUrls
          ? "URL 模式检测到 MVU 依赖，强制启用 Layer1+Layer2"
          : "Zod API 模式检测到 Zod 依赖，强制启用 Layer1+Layer2",
      );
    }
  }

  // Layer 3: Vue — 检测 Vue 3 框架引用
  const needsVue = /\bVue\b|\bv-if\b|\bv-for\b|\bv-model\b|\bcreateApp\b/.test(
    htmlContent,
  );
...
export async function buildInjectionScript(options = {}) {
  ...
  if (needsST) {
    ...
    // 1g. SillyTavern 对象增强
    inlineParts.push(
      generateSTContextEnhancementScript({ userName, charName, messageId }),
    );
    ...
    // 1i2. beiluDebug 调试对象（依赖事件系统/变量系统/stContext，置于其后）
    inlineParts.push(generateBeiluDebugScript());

    // 1i3. BeiluHelper 规范命名空间 + getCurrentEmotion 情感读取契约
    inlineParts.push(generateBeiluHelperScript());
...
export function initSTCompat() {
  ...
  // 初始化变量持久化管理器（受开关保护：关闭时不注册 postMessage 监听器、不挂 __beiluVarStore）
  if (!window.__beiluVarStore && isSTCompatEnabled()) {
    initVariableStore();
  }

  // ★ 预加载 Zod 到父页面（供 iframe 通过 window.parent.z 同步继承）
  // 异步加载，不阻塞初始化；加载完成后 iframe 可通过 earlyScript 继承
  if (
    typeof window.z === "undefined" ||
    !window.z ||
    typeof window.z.object !== "function"
  ) {
    import("https://testingcf.jsdelivr.net/npm/zod@4/+esm")
      .then((zod) => {
        window.z = zod;
        self.z = zod;
        diag.log("Zod 4.x 预加载完成（父页面 — 供 iframe 继承）");
      })
      .catch(() => {
        import("https://cdn.jsdelivr.net/npm/zod@4/+esm")
          .then((zod) => {
            window.z = zod;
            self.z = zod;
            diag.log("Zod 4.x 预加载完成（备用 CDN）");
          })
          .catch((e) => {
            diag.warn(
              "Zod CDN 预加载失败（不影响功能，iframe 将独立加载）:",
              e.message,
            );
          });
      });
  ...
  if (isSTCompatEnabled()) {
    diag.log(
      "ST 兼容层已初始化（开关: 开启）— EventBus + Globals + VarStore + Zod预加载 已挂载",
    );
  } else {
