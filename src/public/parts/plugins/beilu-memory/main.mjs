...
const DEFAULT_INJECTION_PROMPTS = [
  ...
  {
    id: "INJ-3",
    name: "Discord Bot 提示词",
    description:
      "告诉聊天AI在Discord平台上如何回复（格式、语气、提及用户等规则）",
    enabled: false,
    builtin: true,
    deletable: false,
    role: "system",
...
function loadMemoryPresets(username, charName) {
  ...
  if (data && data.presets) {
    ...
    // 自动补全：检查 injection_prompts 是否缺少新增的内置 INJ 条目
    let injSynced = false;
    try {
      ...
      if (fs.existsSync(templatePath)) {
        ...
        if (template && template.injection_prompts) {
          if (!data.injection_prompts) data.injection_prompts = [];
          const existingIds = new Set(data.injection_prompts.map((p) => p.id));
          for (const defaultInj of template.injection_prompts) {
            if (!existingIds.has(defaultInj.id)) {
              data.injection_prompts.push(structuredClone(defaultInj));
              console.log(
                `[beilu-memory] 自动补全: 追加缺失的注入条目 ${defaultInj.id} (${defaultInj.name})`,
              );
              injSynced = true;
            }
          }
      ...
      console.warn(`[beilu-memory] INJ 同步检查失败: ${e.message}`);
    }
    if (injSynced) {
      saveJsonFile(presetsPath, data);
    }
...
function loadMemoryData(username, charName) {
  ...
  const tablesData = loadJsonFileIfExists(path.join(memDir, "tables.json"), {
    tables: structuredClone(DEFAULT_TABLES),
  });
...
function archiveRememberAboutUser(username, charName) {
  ...
  console.log(
    `[beilu-memory] #7 归档了 ${toArchive.length} 条超过3天的记忆 (${charName})`,
  );
...
function archiveCompletedTasks(username, charName, completedRowIndices) {
  ...
  if (!table3 || !completedRowIndices || completedRowIndices.length === 0)
    return { archived: 0 };
...
function endDay(username, charName) {
  ...
  if (table9 && table6) {
    // 将 #6 的内容汇总为一行加入 #9
    const combinedSummary = daySummaryEntries
      .map((row) => `${row[0] || ""} ${row[2] || ""}`)
      .join("; ");
  ...
  steps.push({
    step: 5,
    action: "archive_remember_about_user",
    status: "done",
    archived: step5.archived,
  });
  ...
  steps.push({
    step: 7,
    action: "archive_temp_memory",
    status: "done",
    archived: step7.archived,
    batchFiles: step7.batchFiles,
  });
...
function parseTableEditTags(content) {
  ...
  while ((match = tagRegex.exec(content)) !== null) {
    ...
    const opRegex =
      /(insertRow|updateRow|deleteRow)\s*\(([\s\S]*?)\)(?=\s*(?:insertRow|updateRow|deleteRow|\s*$))/g;
    let op;
...
function executeTableOperations(tables, operations) {
  ...
  for (const op of operations) {
    ...
    if (!table) {
      diag.warn(
        `executeTableOperations: 表格 #${parsed.tableIndex} 不存在 (共${tables.length}个表格)`,
      );
    ...
    if (table.enabled === false) {
      diag.warn(
        `executeTableOperations: 操作目标表格 #${parsed.tableIndex}(${table.name}) 已禁用，操作仍执行（数据层不阻止）`,
      );
    }
    ...
    try {
      switch (op.type) {
        case "insertRow": {
          ...
          diag.debug(
            `executeTableOperations: insertRow(#${parsed.tableIndex}) → row[${table.rows.length - 1}], ${Object.keys(parsed.values).length} 列`,
          );
          successCount++;
          break;
        }
        case "updateRow": {
          if (parsed.rowIndex < 0 || parsed.rowIndex >= table.rows.length) {
            diag.warn(
              `executeTableOperations: updateRow 行 #${parsed.rowIndex} 不存在于表格 #${parsed.tableIndex} (共${table.rows.length}行)`,
            );
            console.warn(
              `[beilu-memory] 行 #${parsed.rowIndex} 不存在于表格 #${parsed.tableIndex}`,
            );
            break;
          }
          const updatedCols = [];
          ...
          diag.debug(
            `executeTableOperations: updateRow(#${parsed.tableIndex}, row${parsed.rowIndex}): ${updatedCols.join(", ")}`,
          );
          successCount++;
          break;
        }
        case "deleteRow": {
          if (parsed.rowIndex < 0 || parsed.rowIndex >= table.rows.length) {
            diag.warn(
              `executeTableOperations: deleteRow 行 #${parsed.rowIndex} 不存在于表格 #${parsed.tableIndex} (共${table.rows.length}行)`,
            );
            console.warn(
              `[beilu-memory] 行 #${parsed.rowIndex} 不存在于表格 #${parsed.tableIndex}`,
            );
          ...
          diag.debug(
            `executeTableOperations: deleteRow(#${parsed.tableIndex}, row${parsed.rowIndex}): ${JSON.stringify(deletedRow).substring(0, 100)}`,
          );
      ...
      try {
        ...
        switch (op.name) {
          case "createFile": {
            ...
            console.log(
              `[beilu-memory] memoryArchive: createFile("${relPath}")`,
            );
            break;
          }
          ...
          case "appendToFile": {
            ...
            console.log(
              `[beilu-memory] memoryArchive: appendToFile("${relPath}")`,
            );
            break;
          }
          ...
          case "updateFile": {
            ...
            console.log(
              `[beilu-memory] memoryArchive: updateFile("${relPath}")`,
            );
            break;
          }
          ...
          case "updateIndex": {
            ...
            console.log(
              `[beilu-memory] memoryArchive: updateIndex("${relPath}")`,
            );
            break;
          }
          ...
          case "moveEntries": {
            if (
              ...
              if (
                fs.existsSync(srcFull) &&
                fs.statSync(srcFull).isDirectory()
              ) {
                ...
                for (const f of files) {
                  ...
                  try {
                    ...
                    console.warn(
                      `[beilu-memory] moveEntries: 移动 ${f} 失败:`,
                      moveErr.message,
                    );
                  }
                }
                ...
                console.log(
                  `[beilu-memory] memoryArchive: moveEntries("${srcRel}" → "${destRel}") ${moved}个文件`,
                );
              } else {
                results.push({
                  op: "moveEntries",
              ...
              const destArrayKey = Array.isArray(destData.entries)
                ...
                : Array.isArray(destData.items)
                  ? "items"
                  : srcArrayKey;
              if (!Array.isArray(destData[destArrayKey]))
                destData[destArrayKey] = [];
              ...
              console.log(
                `[beilu-memory] memoryArchive: moveEntries("${srcRel}" → "${destRel}") ${toMove.length}条`,
              );
            } else {
              results.push({
                op: "moveEntries",
          ...
          case "clearTable": {
            ...
            if (tableIndex >= 0 && tableIndex < tables.length) {
              ...
              console.log(
                `[beilu-memory] memoryArchive: clearTable(${tableIndex})`,
              );
            } else {
              results.push({
                op: "clearTable",
                status: "error",
                error: `表格 #${tableIndex} 不存在`,
              });
            }
            break;
          }
          ...
          case "deleteFile": {
            ...
            console.warn(
              "[beilu-memory] memoryArchive: deleteFile 被拒绝（安全策略）",
            );
            break;
          }
        ...
        console.error(
          `[beilu-memory] memoryArchive: ${op.name} 异常:`,
          e.message,
        );
      }
    }
...
/**
 * 在记忆目录中递归搜索包含关键词的文件
 * @param {string} baseDir - 记忆根目录
 * @param {string} keyword - 搜索关键词（大小写不敏感）
 * @param {object} options - { maxResults: 30, maxFileSize: 50*1024, maxMatchesPerFile: 5 }
 * @returns {Array<{file: string, matchCount: number, matches: Array<{line: number, context: string}>}>}
 */
function searchMemoryFiles(baseDir, keyword, options = {}) {
  const {
    maxResults = 30,
    maxFileSize = 50 * 1024,
    maxMatchesPerFile = 5,
  ...
  walk(baseDir);
  diag.timeEnd("searchMemoryFiles");
  diag.debug(
    `searchMemoryFiles: keyword="${keyword}", ${results.length} 文件匹配, ${totalMatches} 处匹配`,
  );
  if (results.length > 0) {
    diag.snapshot("searchMemoryFiles", {
      keyword,
      fileCount: results.length,
      totalMatches,
      topFiles: results.slice(0, 5).map((r) => `${r.file}(${r.matchCount})`),
    });
  }
  return results;
}
...
/**
 * 获取记忆目录的统计信息
 * @param {string} memDir - 记忆根目录
 * @returns {{totalFiles: number, totalSize: number, layers: object, tablesSummary: string}}
 */
function getMemoryStats(memDir) {
  diag.time("getMemoryStats");
  const stats = {
    totalFiles: 0,
    totalSize: 0,
    layers: {
  ...
  if (fs.existsSync(hotDir)) {
    try {
      ...
      for (const f of hotFiles) {
        if (f.isFile()) {
          try {
            const s = fs.statSync(path.join(hotDir, f.name));
          ...
          try {
            const subFiles = fs.readdirSync(path.join(hotDir, f.name));
            stats.layers.hot.details[f.name + "/"] = `${subFiles.length} files`;
          } catch {
            /* skip */
          }
        }
      ...
      for (const y of years) {
        ...
        try {
          const months = fs
            ...
            .filter((f) => /^\d{2}$/.test(f));
          for (const m of months) {
            stats.layers.warm.months.push(`${y}-${m}`);
          }
        } catch {
          /* skip */
        }
  ...
  if (fs.existsSync(coldDir)) {
    try {
      const years = fs.readdirSync(coldDir).filter((f) => /^\d{4}$/.test(f));
      stats.layers.cold.years = years;
    } catch {
      /* skip */
    }
  }
  ...
  diag.timeEnd("getMemoryStats");
  diag.debug(
    `getMemoryStats: ${stats.totalFiles} 文件, ${(stats.totalSize / 1024).toFixed(1)}KB, hot=${stats.layers.hot.files} warm=${stats.layers.warm.files} cold=${stats.layers.cold.files}`,
  );
  return stats;
}

/**
 * 执行 <memorySearch> 中的文件操作（readFile / listDir / searchKeyword / searchRegex / getStats）
 * P1 多轮检索时，AI 输出 <memorySearch> 标签后由此函数实际读取文件
 * @param {string[]} searchOpsRaw - parseMemorySearchTags() 提取的原始字符串数组
 * @param {string} username
 * @param {string} charName
 * @returns {Array<{op: string, path?: string, keyword?: string, content?: string, entries?: Array, fileMatches?: Array, statsText?: string, error?: string}>}
 */
function executeMemorySearchOps(searchOpsRaw, username, charName) {
  diag.time("executeMemorySearchOps");
  diag.debug(
    `executeMemorySearchOps: ${searchOpsRaw.length} 个搜索块, user=${username}, char=${charName}`,
  );
  const results = [];
  const memDir = getMemoryDir(username, charName);
  const resolvedMemDir = path.resolve(memDir);
  ...
  for (const rawBlock of searchOpsRaw) {
    ...
    // 匹配 readFile("path") / listDir("path") / searchKeyword("keyword"[, "subdir"]) / searchRegex("pattern"[, "subdir"]) / getStats()
    // 容错: 操作名大小写不敏感(gi), 参数支持双引号和单引号
    const opRegex =
      /(readFile|listDir|searchKeyword|searchRegex|getStats)\s*\(\s*(?:["']([^"']*)["'])?(?:\s*,\s*["']([^"']*)["'])?\s*\)/gi;
    // 大小写标准化映射（AI 可能输出 ReadFile / SEARCHKEYWORD 等变体）
    const OP_NORMALIZE = {
      readfile: "readFile",
      listdir: "listDir",
    ...
    while ((match = opRegex.exec(body)) !== null) {
      const [, rawOpType, arg, arg2] = match;
      ...
      if (opType === "getStats") {
        ...
        try {
          ...
          if (stats.layers.warm.months.length > 0) {
            lines.push(`  月份: ${stats.layers.warm.months.join(", ")}`);
          } else {
            lines.push("  (空)");
          }
          lines.push(
          ...
          if (stats.layers.cold.years.length > 0) {
            lines.push(`  年份: ${stats.layers.cold.years.join(", ")}`);
          } else {
            lines.push("  (空)");
          }
          if (stats.tablesSummary) {
            lines.push(`\n表格概况:\n${stats.tablesSummary}`);
          }
          results.push({ op: "getStats", statsText: lines.join("\n") });
        } catch (e) {
          results.push({ op: "getStats", error: e.message });
        }
        continue;
      }

      if (opType === "searchKeyword") {
        // 关键词搜索记忆文件（支持可选的第二个参数指定子目录）
        const keyword = arg || "";
        if (!keyword) {
          results.push({
            op: "searchKeyword",
        ...
        try {
          ...
          if (arg2) {
            const subDir = path.join(memDir, arg2);
            if (
              path.resolve(subDir).startsWith(resolvedMemDir) &&
              ...
              results.push({
                op: "searchKeyword",
                keyword,
                error: `子目录不存在或越界: ${arg2}`,
              });
              continue;
            }
          }
          const fileMatches = searchMemoryFiles(searchDir, keyword);
          // 如果在子目录中搜索，修正文件路径为相对于记忆根目录
          if (arg2 && searchDir !== memDir) {
            for (const fm of fileMatches) {
              fm.file = arg2.replace(/\\/g, "/") + "/" + fm.file;
            }
          }
          // 按时间排序：从文件路径提取日期信息
          sortSearchResultsByDate(fileMatches);
          results.push({ op: "searchKeyword", keyword, fileMatches });
        } catch (e) {
          results.push({ op: "searchKeyword", keyword, error: e.message });
        }
        continue;
      }

      if (opType === "searchRegex") {
        // 正则搜索记忆文件（支持可选的第二个参数指定子目录）
        const pattern = arg || "";
        if (!pattern) {
          results.push({
            op: "searchRegex",
        ...
        try {
          ...
          // 按时间排序
          sortSearchResultsByDate(fileMatches);
          results.push({ op: "searchRegex", keyword: pattern, fileMatches });
        } catch (e) {
          results.push({
            op: "searchRegex",
            keyword: pattern,
      ...
      // readFile / listDir — 原有逻辑
      const relPath = arg || "";
      if (!relPath) {
        results.push({ op: opType, path: "", error: "缺少路径参数" });
        continue;
      }
      const fullPath = path.join(memDir, relPath);
  ...
  diag.timeEnd("executeMemorySearchOps");
  const opSummary = results
    .map((r) => `${r.op}(${r.error ? "❌" : "✓"})`)
    .join(", ");
  diag.debug(
    `executeMemorySearchOps: 完成 ${results.length} 个操作: ${opSummary}`,
  );
  if (results.some((r) => r.error)) {
    diag.snapshot("executeMemorySearchOps.errors", {
      errors: results
        .filter((r) => r.error)
        .map((r) => ({ op: r.op, error: r.error })),
    });
  }
  return results;
}
...
/**
 * 搜索结果按时间排序（从旧到新）
 * 从文件路径提取日期信息（如 warm/2026/02/15_summary.json → "2026/02/15"）
 * hot层文件排在最后（最新），无法提取日期的文件排在中间
 * @param {Array<{file: string}>} fileMatches - 搜索结果数组（原地排序）
 */
function sortSearchResultsByDate(fileMatches) {
 ...
 * 带 token 限制：总字符数超过 MAX_INJECTION_CHARS 时截断
 * @param {Array} searchResults - executeMemorySearchOps() 的返回值
 * @param {number} [charLimit] - 字符数上限（默认 MAX_INJECTION_CHARS）
 * @returns {string}
 */
function formatSearchResultsForAI(searchResults, charLimit) {
  if (!searchResults || searchResults.length === 0) return "(无搜索结果)";
  const limit = charLimit || MAX_INJECTION_CHARS;
  let totalChars = 0;
  let truncated = false;
  ...
  for (const r of searchResults) {
    ...
    if (r.error) {
      ...
      for (const entry of r.entries || []) {
        const line = `  ${entry.isDir ? "📂" : "📄"} ${entry.name}`;
        totalChars += line.length;
        if (totalChars > limit) {
          truncated = true;
          break;
    ...
    } else if (r.op === "searchKeyword" || r.op === "searchRegex") {
      const icon = r.op === "searchKeyword" ? "🔍" : "🔎";
      const header = `\n${icon} ${r.op}("${r.keyword}"):`;
      totalChars += header.length;
      if (totalChars > limit) {
        truncated = true;
      ...
      if (r.fileMatches && r.fileMatches.length > 0) {
        ...
        for (const fm of r.fileMatches) {
          if (truncated) break;
          const fmHeader = `  📄 ${fm.file} (${fm.matchCount}处匹配):`;
  ...
  if (truncated) {
    lines.push(
      `\n⚠️ 搜索结果已截断（超过${Math.round(limit / 500)}K token 上限）`,
    );
  }
  lines.push("[/记忆文件搜索结果]");
  return lines.join("\n");
 ...
 */
async function runMemoryPresetAI(
  username,
  charName,
  preset,
  memData,
  ...
  for (const prompt of preset.prompts || []) {
    ...
    content = content
      ...
      .replace(
        /\{\{presetList\}\}/g,
        preset.preset_switch_auto === false ? "" : getPresetListForP1(preset),
      );
  ...
  console.log(
    `[beilu-memory] ===== P${preset.id} 实际构建的 messages (共${messages.length}条) =====`,
  );
  for (let mi = 0; mi < messages.length; mi++) {
    const msg = messages[mi];
    const preview = (msg.content || "").substring(0, 120).replace(/\n/g, "\\n");
    console.log(
      `[beilu-memory]   [${mi}] role=${msg.role}, len=${(msg.content || "").length}, preview: ${preview}`,
    );
  }
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    if (archiveOps.length > 0) {
      ...
      console.log(
        `[beilu-memory] 记忆AI(${preset.id}) 第${round}轮: memoryArchive ${archiveOkCount}/${archiveResults.length} 操作成功`,
      );
    }
    ...
    // <memoryNote>
    processedContent = parseMemoryNoteTags(
      processedContent,
      username,
      charName,
    );

    // 提取 <thinking> / <think> 并从 processedContent 中清除
    let roundThinking = "";
    ...
    if (thinkingMatch) {
      ...
      if (thinkMatch) {
        roundThinking = thinkMatch[1].trim();
      }
    }
    // 从 processedContent 中清除 thinking 标签，防止泄漏到 finalReply → 聊天AI注入
    ...
    console.log(
      `[beilu-memory] 记忆AI(${preset.id}) 第${round}轮回复: ${replyContent.length}字符, 搜索操作: ${searchOps.length}个`,
    );
 ...
 */
async function triggerP1Retrieval(
  username,
  charName,
  memData,
  displayCharName,
  ...
  try {
    ...
    const isNoResult =
      replyLower.length < 5 ||
      noResultKeywords.some((kw) => replyLower.includes(kw));
    if (isNoResult) {
      console.log(
        `[beilu-memory] P1 判定无实质内容: "${replyLower.substring(0, 30)}"`,
      );
      // 推送完成状态（无实质内容）
      pushMemoryAIOutput({
        presetId: "P1",
...
/**
 * 聊天AI搜索结果缓存（Phase 2）
 * 当聊天AI在回复中使用 <memorySearch> 标签时，ReplyHandler 执行搜索并将结果存入此 Map。
 * 下一轮 GetPrompt 时读取并注入到 depthInjections 中（一次性使用）。
 * Key: "username/charName"
 * @type {Map<string, {results: string, timestamp: string}>}
 */
const pendingChatSearchResults = new Map();

/** 注入 token 上限（约10万token ≈ 50000中文字符） */
const MAX_INJECTION_CHARS = 50000;
...
const pluginExport = {
  Load: async ({ router, username }) => {
    ...
    try {
      ...
      console.warn(
        "[beilu-memory] auth.mjs 未能加载，将使用默认用户名:",
        e.message,
      );
    }
    ...
    if (router) {
      router.get(
        "/api/parts/plugins\\:beilu-memory/config/getdata",
        async (req, res) => {
          try {
            let username;
            if (_getUserByReq)
              try {
                username = (await _getUserByReq(req)).username;
              } catch {
                /* fallback */
              }
            const query = {
      ...
      SetData: async (data, args) => {
        ...
        switch (data._action) {
          ...
          case "updateTable": {
            ...
            if (tableIdx >= 0 && tableIdx < memData.tables.length) {
              ...
              if (data.columns !== undefined) {
                ...
                changes.push(
                  `columns: [${oldCols}]→[${data.columns.join(",")}]`,
                );
              }
              ...
              if (data.name !== undefined) {
                ...
                if (oldName !== data.name)
                  changes.push(`name: "${oldName}"→"${data.name}"`);
              }
              if (data.enabled !== undefined) {
                ...
                if (targetTable.required) {
                  ...
                  if (!data.enabled)
                    changes.push(`enabled: 拒绝禁用(required)`);
                } else {
                  targetTable.enabled = !!data.enabled;
                  if (oldEnabled !== targetTable.enabled)
                    changes.push(
                      `enabled: ${oldEnabled}→${targetTable.enabled}`,
                    );
                }
              }
              if (changes.length > 0) {
                diag.log(
                  `updateTable: #${targetTable.id}(${targetTable.name}) by ${username}/${charName}: ${changes.join(", ")}`,
                );
              }
              saveTablesData(username, charName);
            }
          ...
          case "addTable": {
            ...
            diag.log(
              `addTable: #${newId}(${newName}), ${newCols.length} 列, by ${username}/${charName}`,
            );
            saveTablesData(username, charName);
          }
          case "removeTable": {
            ...
            if (idx >= 0 && idx < memData.tables.length) {
              ...
              if (removedTable.required) {
                diag.warn(
                  `removeTable: 拒绝删除 required 表格 #${removedTable.id}(${removedTable.name})`,
                );
                break; // 不能删除必须的表格
              }
              diag.log(
                `removeTable: #${removedTable.id}(${removedTable.name}), ${removedTable.rows?.length || 0} 行数据, by ${username}/${charName}`,
              );
          ...
          case "updateMemoryPreset": {
            ...
            if (data.description !== undefined)
              preset.description = String(data.description);
            if (data.trigger !== undefined)
              preset.trigger = String(data.trigger);
            ...
            // 预设切换配置（仅 P1 使用）
            if (data.preset_switch_auto !== undefined)
              preset.preset_switch_auto = !!data.preset_switch_auto;
            if (data.preset_switch_entries !== undefined)
              preset.preset_switch_entries = data.preset_switch_entries;
          ...
          case "updatePresetPrompt": {
            ...
            if (data.role !== undefined && !prompt.builtin)
              prompt.role = data.role;
            if (data.content !== undefined)
              prompt.content = String(data.content);
          ...
          case "addPresetPrompt": {
            ...
            // 在 {{chat_history}} 之前插入
            const chatHistoryIdx = preset.prompts.findIndex(
              (p) => p.builtin && p.content === "{{chat_history}}",
            );
            if (chatHistoryIdx >= 0) {
          ...
          case "reorderPresetPrompts": {
            ...
            for (const identifier of order) {
              const found = preset.prompts.find(
          ...
          case "updateInjectionPrompt": {
            ...
            if (data.description !== undefined)
              inj.description = String(data.description);
            if (data.role !== undefined) inj.role = data.role;
            if (data.depth !== undefined)
              inj.depth = parseInt(data.depth, 10) || 0;
            if (data.order !== undefined)
              inj.order = parseInt(data.order, 10) || 0;
          ...
          case "addInjectionPrompt": {
            ...
            const maxOrder = injPrompts.reduce(
              (max, p) => Math.max(max, p.order || 0),
              0,
            );
            const newInj = {
              id: newId,
              name: data.name || "新注入条目",
              description: data.description || "",
              enabled: data.enabled !== undefined ? !!data.enabled : true,
              builtin: false,
              deletable: true,
              role: data.role || "system",
              depth: parseInt(data.depth, 10) || 999,
              order: parseInt(data.order, 10) || maxOrder + 100,
              autoMode: data.autoMode || "always",
              content: data.content || "",
            };
            ...
            saveMemoryPresets(username, charName, presetsData);
            console.log(
              `[beilu-memory] 添加注入提示词: ${newId} (${newInj.name})`,
            );
            return { success: true, id: newId, injection: newInj };
          }
          case "deleteInjectionPrompt": {
            // 删除注入提示词条目（仅 deletable=true 的条目可删除）
            const injId = data.injectionId;
            if (!injId) return { success: false, error: "缺少 injectionId" };
            const injPrompts = presetsData.injection_prompts || [];
            const injIdx = injPrompts.findIndex((p) => p.id === injId);
            if (injIdx === -1)
              return { success: false, error: `未找到注入条目 ${injId}` };
            const target = injPrompts[injIdx];
            if (!target.deletable)
              return { success: false, error: `内置条目 ${injId} 不可删除` };
            injPrompts.splice(injIdx, 1);
            presetsData.injection_prompts = injPrompts;
            saveMemoryPresets(username, charName, presetsData);
            console.log(
              `[beilu-memory] 删除注入提示词: ${injId} (${target.name})`,
            );
            return { success: true, deleted: injId };
          }
          case "previewMemoryPreset": {
            ...
            // 生成热记忆文本用于预览
            let previewHotMemory = readHotMemoryForInjection(
              username,
              charName,
            );
            if (previewHotMemory) {
            ...
            const totalChars = previewPrompts.reduce(
              (sum, p) => sum + p.charCount,
              0,
            );
          ...
          case "previewInjectionPrompt": {
            ...
            let previewContent = inj.content
              ...
              .replace(
                /\{\{idle_duration\}\}/g,
                _tmPreview.idle_duration || "（预览模式）",
              )
              .replace(
                /\{\{lasttime\}\}/g,
                _tmPreview.lasttime || "（预览模式）",
              )
              .replace(
                /\{\{lastdate\}\}/g,
                _tmPreview.lastdate || "（预览模式）",
              );
          ...
          case "archiveHotToWarm": {
            ...
            return {
              success: true,
              remember_archived: r7.archived,
              forever_archived: r8.archived,
            };
          }
          ...
          case "writeMemoryFile": {
            ...
            try {
              ...
              return {
                success: true,
                filePath,
                size: Buffer.byteLength(writeContent, "utf8"),
              };
            } catch (e) {
            ...
            try {
              ...
              function sanitizePresetsForExport(jsonStr) {
                try {
                  ...
                  console.warn(
                    "[beilu-memory] exportMemory: 清洗 _memory_presets.json 失败:",
                    e.message,
                  );
              ...
              function sanitizeConfigForExport(jsonStr) {
                try {
                  ...
                  console.warn(
                    "[beilu-memory] exportMemory: 清洗 _config.json 失败:",
                    e.message,
                  );
                  return jsonStr;
                }
              ...
              function addDirToZip(dir, zipFolder, relBase) {
                ...
                try {
                  ...
                  console.warn(
                    `[beilu-memory] exportMemory: 无法读取目录 ${relBase || "/"}: ${e.message}`,
                  );
                  return;
                }
                for (const entry of entries) {
                  ...
                  if (
                    ...
                    entry.name.endsWith(".import_bak")
                  )
                    continue;
                  const fullPath = path.join(dir, entry.name);
                  if (entry.isDirectory()) {
                    ...
                    try {
                      ...
                      console.warn(
                        `[beilu-memory] exportMemory: 读取失败 ${relBase ? relBase + "/" : ""}${entry.name}: ${e.message}`,
                      );
                    }
                  }
                }
              ...
              console.log(
                `[beilu-memory] 导出记忆: ${charName}, ${fileCount} 个文件 → zip`,
              );
            ...
            try {
              ...
              if (data.zipBase64) {
                ...
                for (const [relPath, zipEntry] of Object.entries(zip.files)) {
                  ...
                  try {
                    ...
                    if (backupExisting && fs.existsSync(fullPath)) {
                      try {
                        fs.copyFileSync(fullPath, fullPath + ".import_bak");
                      } catch {
                        /* ignore */
                      }
                    }
                ...
                if (!importData || !importData.files) {
                  return {
                    success: false,
                    error: "无效的导入数据：缺少 files 字段或 zipBase64",
                  };
                }
                if (importData._format !== "beilu-memory-export") {
                  return {
                    success: false,
                    error: "无效的导入数据：格式标识不匹配",
                  };
                }
              ...
              console.log(
                `[beilu-memory] 导入记忆: ${charName}, 成功 ${imported}, 跳过 ${skipped}`,
              );
              return {
                success: true,
                imported,
          ...
          case "importPresets": {
            ...
            const importData = data.importData;
            if (!importData)
              return { success: false, error: "缺少 importData" };
            if (importData._format !== "beilu-memory-presets-export") {
              return {
                success: false,
                error: "无效的预设文件：格式标识不匹配",
              };
            }
            if (
              !Array.isArray(importData.presets) ||
              !Array.isArray(importData.injection_prompts)
            ) {
              return {
                success: false,
                error: "无效的预设文件：缺少 presets 或 injection_prompts 数组",
              };
            }
            ...
            if (backupExisting) {
              ...
              if (fs.existsSync(presetsPath)) {
                try {
                  fs.copyFileSync(presetsPath, presetsPath + ".import_bak");
                } catch {
                  /* ignore */
                }
              }
            ...
            console.log(
              `[beilu-memory] 预设已导入: ${importData.presets.length} 个预设, ${importData.injection_prompts.length} 个注入条目`,
            );
            return {
              success: true,
          ...
          case "runMemoryPreset": {
            ...
            const preset = presetsData.presets.find((p) => p.id === presetId);
            if (!preset)
              return { success: false, error: `未找到预设 ${presetId}` };
            ...
            try {
              ...
              if (presetId === "P1" && result.reply) {
                ...
                console.log(
                  `[beilu-memory] P1 检索结果已缓存，将在下次聊天中注入 (${result.reply.length}字符)`,
                );
              }
              ...
              console.error(
                `[beilu-memory] runMemoryPreset(${presetId}) 失败:`,
                e.message,
              );
              // 推送错误到输出队列
              if (!data.dryRun) {
          ...
          case "getMemoryAIOutput": {
            ...
            const sinceId =
              data.sinceId !== undefined && data.sinceId !== null
                ? Number(data.sinceId)
                : null;
          ...
          case "getDiagSnapshot": {
            ...
            return {
              ...
              p1CacheLength: lastP1Result
                ? (lastP1Result.reply || "").length
                : 0,
              p1CacheTimestamp: lastP1Result?.timestamp || null,
              p1CacheContent: lastP1Result?.reply || null,
              isP1Running,
              enabledPresets: presetsForDiag
                .filter((p) => p.enabled)
                .map((p) => p.id),
              enabledInjections: injPromptsForDiag
                .filter((p) => p.enabled)
                .map((p) => p.id),
              ...
              cooldownConfig:
                memData.config?.preset_switch?.cooldown_rounds ?? 5,
            };
          }
          ...
          case "updateConfig": {
            ...
            const memDir = getMemoryDir(username, charName);
            const configPath = path.join(memDir, "_config.json");
            const currentConfig = loadJsonFileIfExists(configPath, {
              enabled: true,
            });
            ...
            console.log(
              `[beilu-memory] 配置已更新 (${charName}):`,
              Object.keys(data).filter(
                (k) => k !== "_action" && k !== "username" && k !== "charName",
              ),
            );
    ...
    chat: {
      ...
      GetPrompt: async (arg) => {
        ...
        try {
          ...
          const _cooldownConfig =
            data.config?.preset_switch?.cooldown_rounds ?? 5;
              ...
              .replace(
                ...
                (() => {
                  ...
                  return arg.chat_log
                    ...
                    .map((m) => {
                      const role =
                        m.role === "user" ? userName : displayCharName;
          ...
          // fake-send 模式下跳过 P1 检索（避免提示词查看器触发真实AI调用）
          {
            ...
            if (retrievalConfig.auto_trigger && !arg.isFakeSend) {
              ...
              if (p1Preset && p1Preset.enabled) {
                // 构建聊天记录
                let chatHistory = "";
                const chatHistoryCount =
                  retrievalConfig.chat_history_count || 5;
                if (arg?.chat_log && Array.isArray(arg.chat_log)) {
                ...
                try {
                  ...
                  if (!_isP1NoResult) {
                    ...
                    console.log(
                      `[beilu-memory] P1 检索结果已注入到本轮对话 (${result.reply.length}字符)`,
                    );
                  } else {
                    console.log(
            ...
            if (
              !_presetSwitchExecuted &&
              presetSwitchCooldown.has(_cooldownKey)
            ) {
            ...
            // Phase 2：聊天AI搜索结果注入
            // 上一轮聊天AI在回复中使用了 <memorySearch>，搜索结果在此注入
            if (!arg.isFakeSend) {
              const chatSearchKey = `${username}/${charName}`;
              const pendingSearch = pendingChatSearchResults.get(chatSearchKey);
              if (pendingSearch) {
                const searchContent = `[上轮聊天AI搜索结果 (${pendingSearch.timestamp})]\n${pendingSearch.results}\n[/上轮聊天AI搜索结果]`;
                depthInjections.push({
                  id: "CHAT_SEARCH_RESULT",
                  role: "system",
                  content: searchContent,
                  depth: 0,
                  order: 2,
                });
                textEntries.push({ content: searchContent, important: 5 });
                pendingChatSearchResults.delete(chatSearchKey);
                console.log(
                  `[beilu-memory] GetPrompt: 注入聊天AI搜索结果 (${searchContent.length}字符)`,
                );
              }
            }
          }
          ...
          if (textEntries.length === 0) {
            const tableText = tablesToPromptText(
              data.tables,
              displayCharName,
              userName,
            );
          ...
          // 注入日志：正常模式
          const _p1WasInjected = depthInjections.some(
            (d) => d.id === "P1_RETRIEVAL",
          );
          injectionLog.push({
            timestamp: new Date().toISOString(),
      ...
      ReplyHandler: async (reply, args) => {
        ...
        try {
          ...
          // 1. 解析 <tableEdit>
          const { operations, cleanContent: afterTableEdit } =
            parseTableEditTags(content);
          content = afterTableEdit;

          if (operations.length > 0) {
            const data = loadMemoryData(username, charName);
            const successCount = executeTableOperations(
              data.tables,
              operations,
            );
            if (successCount > 0) {
              saveTablesData(username, charName);
              console.log(
                `[beilu-memory] 执行了 ${successCount}/${operations.length} 个表格操作 (${charName})`,
              );
            }
          }
          ...
          const { archiveOps, cleanContent: afterArchive } =
            parseMemoryArchiveTags(content);
          ...
          if (archiveOps.length > 0) {
            ...
            console.log(
              `[beilu-memory] ReplyHandler: memoryArchive ${archiveOkCount}/${archiveResults.length} 操作成功 (${charName})`,
            );
          }

          // 3. 解析 <memorySearch>（Phase 2：聊天AI也能搜索记忆）
          const { searchOps: chatSearchOps, cleanContent: afterSearch } =
            parseMemorySearchTags(content);
          content = afterSearch;
          if (chatSearchOps.length > 0) {
            try {
              const chatSearchResults = executeMemorySearchOps(
                chatSearchOps,
                username,
                charName,
              );
              const formattedResults =
                formatSearchResultsForAI(chatSearchResults);
              if (formattedResults && formattedResults !== "(无搜索结果)") {
                const cacheKey = `${username}/${charName}`;
                pendingChatSearchResults.set(cacheKey, {
                  results: formattedResults,
                  timestamp: new Date().toISOString(),
                });
                console.log(
                  `[beilu-memory] ReplyHandler: 聊天AI搜索 ${chatSearchOps.length} 个操作，结果已缓存待下轮注入 (${formattedResults.length}字符)`,
                );
              }
            } catch (e) {
              console.warn(
                `[beilu-memory] ReplyHandler: 聊天AI搜索执行失败:`,
                e.message,
              );
            }
          }
