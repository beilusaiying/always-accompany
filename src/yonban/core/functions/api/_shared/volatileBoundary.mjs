...
// 尾部回看窗口（条）：只在尾部 N 条内扫易变起点——启发式领域假设（每轮追加 user+assistant
// 两条，易变注入贴尾），窗口过大会把历史正文里偶含的标签误判为边界。
...
export function isVolatileBoundaryMsg(msg) {
  ...
  if (id.includes("-data") || sec === "afterChat" || sec === "injectionBelow") return true;
  return VOLATILE_TAG_RE.test(msgText(msg));
}
...
export function findVolatileStart(messages, { lookback = VOLATILE_LOOKBACK } = {}) {
  ...
  for (let i = messages.length - 1; i >= Math.max(0, messages.length - lookback); i--) {
    if (isVolatileBoundaryMsg(messages[i])) volStart = i;
  }
  return volStart;
}
